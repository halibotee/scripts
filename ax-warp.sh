...
uninstall() {
  ...
  if [ -x "$(type -p wireproxy)" ]; then
    ...
    if [ "$SYSTEM" = "Alpine" ]; then
      ...
      systemctl stop wireproxy > /dev/null 2>&1
      systemctl disable wireproxy > /dev/null 2>&1
      rm -f /lib/systemd/system/wireproxy.service
      systemctl daemon-reload > /dev/null 2>&1
    fi
    rm -f /usr/bin/wireproxy /etc/wireguard/proxy.conf
    info " ✓ WireProxy 卸载完成"
  fi
  ...
  rm -f /etc/wireguard/warp-account.conf \
        ...
        /etc/wireguard/info.log \
        /etc/wireguard/license \
        /etc/wireguard/proxy.conf \
        /etc/wireguard/NonGlobalUp.sh \
        /etc/wireguard/NonGlobalDown.sh
  
  # 如果目录为空则删除
  if [ -d /etc/wireguard ] && [ -z "$(ls -A /etc/wireguard 2>/dev/null)" ]; then
    rmdir /etc/wireguard
    info " ✓ 已删除空目录 /etc/wireguard"
  fi
