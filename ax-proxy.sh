...
generate_hy2_subscription_link() {
    ...
    # 只提取 auth 区块下的 password
    local password=$(awk '/^auth:/,/^[a-z]/ {if (/^  password:/) print $2}' "$conf" | head -n 1)
...
view_chain_client_config() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        # 只提取 auth 区块下的 password
        local hy2_password=$(awk '/^auth:/,/^[a-z]/ {if (/^  password:/) print $2}' "$main_conf_path" | head -n 1)
