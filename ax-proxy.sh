...
generate_instance_display_name() {
    local type=$1 id=$2; local domain="" conf_file=""
    case "$type" in
    ...
    if [[ -f "$conf_file" ]]; then
        domain=$(grep -oE '[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)+\.[a-zA-Z]{2,}' "$conf_file" 2>/dev/null | grep -v '^[0-9.]*$' | head -1)
    fi
    if [[ -z "$domain" ]]; then
    ...
    case "$type" in
        hysteria2) echo "HY2_${domain}_${id}" ;;
        udp2raw) echo "UDP2RAW_${domain}_${id}" ;;
        kcptun) echo "KCP_${domain}_${id}" ;;
        xray_reality) echo "VLESS_${domain}_${id}" ;;
        xray_mkcp) echo "VLESS_mKCP_${domain}_${id}" ;;
        xray_ss) echo "SS_${domain}_${id}" ;;
        hy2_chain) echo "HY2_UDP_${domain}_${id}" ;;
        vless_chain) echo "VLESS_KCP_UDP_${domain}_${id}" ;;
        ss_3_chain_chain) echo "SS_KCP_UDP_${domain}_${id}" ;;
        *) echo "${type}_${id}" ;;
    esac
}
