#!/bin/bash
# 支持Xray(VLESS/SS)、Hysteria2、UDP2RAW、KCPTUN的部署与串联
...
# -----------------------------------------------------------------------------
# Hysteria2 混淆（obfs）配置模板 (YAML)
# -----------------------------------------------------------------------------
read -r -d '' HYSTERIA2_OBFS_TEMPLATE <<'EOM'
obfs:
  type: salamander
  salamander:
...
read -r -d '' XRAY_VLESS_MKCP_TEMPLATE <<'EOM'
{
    ...
    "inbounds": [{
        ...
        "streamSettings": {
            "network": "kcp",
            "kcpSettings": {
                "mtu": __MKCP_MTU__, "tti": 30, "uplinkCapacity": 25, "downlinkCapacity": 100, "congestion": __MKCP_CONGESTION__, "readBufferSize": 4, "writeBufferSize": 4,
...
# 检查是否已安装任何实例
# -----------------------------------------------------------------------------
is_installed(){ find "$KCP_INSTALL_DIR" -maxdepth 1 -name '*.json' 2>/dev/null | grep -q . || find "$UDP2RAW_INSTALL_DIR" -maxdepth 1 -name '*.conf' 2>/dev/null | grep -q . || find "$HY2_INSTALL_DIR" -maxdepth 1 -name '*.yaml' 2>/dev/null | grep -q . || find "$XRAY_INSTALL_DIR" -maxdepth 1 -name '*.json' 2>/dev/null | grep -q .; }
...
[Install]
WantedBy=multi-user.target"
    local hy2_hash=$(echo "$hy2_content" | md5sum 2>/dev/null | cut -d' ' -f1)
    existing_hash=$(md5sum "$HY2_TEMPLATE_FILE" 2>/dev/null | cut -d' ' -f1)
    if [[ "$hy2_hash" != "$existing_hash" ]]; then
        echo "$hy2_content" > "$HY2_TEMPLATE_FILE"; need_reload=true
    fi

    local xray_content="[Unit]
...
[Install]
WantedBy=multi-user.target"
    local xray_hash=$(echo "$xray_content" | md5sum 2>/dev/null | cut -d' ' -f1)
    existing_hash=$(md5sum "$XRAY_TEMPLATE_FILE" 2>/dev/null | cut -d' ' -f1)
    if [[ "$xray_hash" != "$existing_hash" ]]; then
        echo "$xray_content" > "$XRAY_TEMPLATE_FILE"; need_reload=true
    fi
...
generate_instance_display_name() {
    ...
    case "$type" in
        ...
        xray_mkcp) echo "VLESS_mKCP_${ip}_${id}" ;;
        xray_ss) echo "SS_${ip}_${id}" ;;
        hy2_chain) echo "HY2_UDP_${ip}_${id}" ;;
        vless_chain) echo "VLESS_KCP_UDP_${ip}_${id}" ;;
...
create_new_instance() {
    ...
    case "$type" in
        hysteria2) 
            ...
            use_acme=${use_acme:-n} # Default to no
            ...
            # 混淆配置
            cyan "--- obfs（混淆）配置 ---"
            read -p "是否开启混淆（Salamander）? [y/N] (默认 N): " enable_obfs
            enable_obfs=${enable_obfs:-n}
            
            if [[ "$enable_obfs" == "y" || "$enable_obfs" == "Y" ]]; then
                local obfs_password=$(generate_strong_password)
                green "已自动生成混淆密码: $obfs_password"
                # 生成混淆配置块并赋值给占位符
                replacements["__OBFS_CONFIG__"]="${HYSTERIA2_OBFS_TEMPLATE//__OBFS_PASSWORD__/${obfs_password}}"
            else
                # 不启用混淆，占位符替换为空
                replacements["__OBFS_CONFIG__"]=""
		fi
...
create_new_xray_instance() {
    ...
    if [[ "$type" == "xray_reality" ]]; then
        ...
        read -p "MTU (默认 1200): " mkcp_mtu
        mkcp_mtu=${mkcp_mtu:-1200}
...
generate_hy2_subscription_link() {
    ...
    # 只提取 auth 区块下的 password（第一次出现的带缩进的 password）
    local password=$(grep -m 1 "^  password:" "$conf" | awk '{print $2}')
    ...
    # 从配置文件中提取混淆密码（如果存在）
    # 匹配 salamander 下的 password 字段
    obfs_password=$(grep -A2 'type: salamander' "$conf" 2>/dev/null | grep -Po '(?<=password: ).*' | tr -d '[:space:]' | head -n 1)
    
    if [[ -n "$obfs_password" ]]; then
        obfs_param="&obfs=salamander&obfs-password=${obfs_password}"
    fi

    echo "hysteria2://${password}@${ip}:${port}?sni=${sni}&insecure=${insecure_flag}${obfs_param}#hy2_${host_label}_${id}"
}
...
manage_chain_instance_3() {
    ...
    while true; do
        ...
        echo "----------------------------------"; echo "1) 启动/重启此串联"; echo "2) 停止此串联"; echo "3) 查看客户端配置"; echo "4) 查看 SS (Xray) 日志"; echo "5) 查看 KCPTUN 日志"; echo "6) 查看 UDP2RAW 日志"; echo "7) 编辑 SS (Xray) 配置文件"; echo "8) 编辑 KCPTUN 配置文件"; echo "9) 编辑 UDP2RAW 配置文件"; echo "10) 修改实例名称"; echo "99) 彻底删除此串联"; echo "0) 返回"
...
view_chain_client_config() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        # 只提取 auth 区块下的 password（第一次出现的带缩进的 password）
        local hy2_password=$(grep -m 1 "^  password:" "$main_conf_path" | awk '{print $2}')
        ...
        # 从配置文件中提取混淆密码（如果存在）
        obfs_password=$(grep -A2 'type: salamander' "$main_conf_path" 2>/dev/null | grep -Po '(?<=password: ).*' | tr -d '[:space:]' | head -n 1)
...
start_new_chain_instance() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        if [[ "$enable_obfs" == "y" || "$enable_obfs" == "Y" ]]; then
            ...
            green "已自动生成混淆密码: $obfs_password"
            # 生成混淆配置块
            obfs_config="${HYSTERIA2_OBFS_TEMPLATE//__OBFS_PASSWORD__/${obfs_password}}"
		fi
		# ------------------------------------
        ...
        main_config="${main_config//__OBFS_CONFIG__/${obfs_config}}"
...
manage_chain_instance() {
    ...
    while true; do
        ...
        echo "----------------------------------"; echo "1) 启动/重启此串联"; echo "2) 停止此串联"; echo "3) 查看客户端配置"; echo "4) 查看 ${title} 日志"; echo "5) 查看 UDP2RAW 日志"; echo "6) 编辑 ${title} 配置文件"; echo "7) 编辑 UDP2RAW 配置文件"; echo "8) 修改实例名称"; echo "99) 彻底删除此串联"; echo "0) 返回"
...
check_for_updates(){
    ...
    for f in "$KCP_INSTALL_DIR"/kcptun_*.json "$UDP2RAW_INSTALL_DIR"/udp2raw_*.conf \
        ...
        case "$base" in
            ...
            hy2_*.yaml)     srv="ax-hysteria2@${base#hy2_}"; srv="${srv%.yaml}" ;;
            xray_*.json)    srv="ax-xray@${base#xray_}"; srv="${srv%.json}" ;;
        esac
...
uninstall_all() {
    ...
    for srv in ax-kcptun@* ax-udp2raw@* ax-xray@* ax-hysteria2@* hysteria-server@*; do systemctl stop "$srv" 2>/dev/null; systemctl disable "$srv" 2>/dev/null; done
    green "服务已停止并禁用。"
