...
install_dependencies_and_programs(){
    ...
    local warp_ok=true; if [[ ! -f "ax-warp.sh" ]]; then warp_ok=false; script_download_needed=true; fi
...
install_sys_opt() {
    ...
    # 确保脚本存在
    if ! ensure_ax_script "ax-optz.sh"; then
        red "错误：无法下载 ax-optz.sh"
        return 1
    fi
...
install_warp_yg() {
    echo "----------------------------------------------------------------"
    log "即将开始运行 WARP 管理脚本..."

    # 确保脚本存在
    if ! ensure_ax_script "ax-warp.sh"; then
        red "错误：无法下载 ax-warp.sh"
        return 1
    fi
...
install_acme_tool() {
    echo "----------------------------------------------------------------"
    log "即将开始运行 ACME 证书管理脚本..."
    
    # 确保脚本存在
    if ! ensure_ax_script "ax-acme.sh"; then
        red "错误：无法下载 ax-acme.sh"
        return 1
    fi
