...
CFWARP_URL="https://raw.githubusercontent.com/halibotee/scripts/main/ax-warpsocket5.sh"    # WARP 配置脚本
...
install_dependencies_and_programs(){
    ...
    local acme_ok=true; if [[ ! -f "ax-acme.sh" ]]; then acme_ok=false; script_download_needed=true; fi
    local warp_ok=true; if [[ ! -f "CFwarp.sh" ]]; then warp_ok=false; script_download_needed=true; fi
    local optz_ok=true; if [[ ! -f "ax-optz.sh" ]]; then optz_ok=false; script_download_needed=true; fi
    ...
    if [[ "$script_download_needed" == false ]]; then
        ...
        if [[ "$acme_ok" == false ]]; then
            cyan "  » 下载 ax-acme.sh"
            download_with_retry "$AX_ACME_URL" "ax-acme.sh" >/dev/null 2>&1 && chmod +x "ax-acme.sh" && green "    ✓ ax-acme.sh 下载完成" || yellow "    ⚠ ax-acme.sh 下载失败"
        fi
        
        if [[ "$warp_ok" == false ]]; then
            cyan "  » 下载 CFwarp.sh"
            download_with_retry "$CFWARP_URL" "CFwarp.sh" >/dev/null 2>&1 && chmod +x "CFwarp.sh" && green "    ✓ CFwarp.sh 下载完成" || yellow "    ⚠ CFwarp.sh 下载失败"
        fi
        
        if [[ "$optz_ok" == false ]]; then
            cyan "  » 下载 ax-optz.sh"
            download_with_retry "$AX_OPTZ_URL" "ax-optz.sh" >/dev/null 2>&1 && chmod +x "ax-optz.sh" && green "    ✓ ax-optz.sh 下载完成" || yellow "    ⚠ ax-optz.sh 下载失败"
        fi
...
check_for_updates(){
    ...
    if [[ -f "ax-acme.sh" ]]; then
        cyan "  » 更新 ax-acme.sh"
        download_with_retry "$AX_ACME_URL" "ax-acme.sh" >/dev/null 2>&1 && chmod +x "ax-acme.sh" && green "    ✓ ax-acme.sh 更新完成" && script_updated=true || yellow "    ⚠ ax-acme.sh 更新失败"
    fi
    
    # 更新 CFwarp.sh
    if [[ -f "CFwarp.sh" ]]; then
        cyan "  » 更新 CFwarp.sh"
        download_with_retry "$CFWARP_URL" "CFwarp.sh" >/dev/null 2>&1 && chmod +x "CFwarp.sh" && green "    ✓ CFwarp.sh 更新完成" && script_updated=true || yellow "    ⚠ CFwarp.sh 更新失败"
    fi
    
    # 更新 ax-optz.sh
    if [[ -f "ax-optz.sh" ]]; then
        cyan "  » 更新 ax-optz.sh"
        download_with_retry "$AX_OPTZ_URL" "ax-optz.sh" >/dev/null 2>&1 && chmod +x "ax-optz.sh" && green "    ✓ ax-optz.sh 更新完成" && script_updated=true || yellow "    ⚠ ax-optz.sh 更新失败"
    fi
...
# -----------------------------------------------------------------------------
install_sys_opt() {
    run_local_script "ax-optz.sh" "VPS 优化脚本" "$AX_OPTZ_URL"
}
...
# -----------------------------------------------------------------------------
install_warp_yg() {
    run_local_script "CFwarp.sh" "WARP 管理脚本" "$CFWARP_URL"
}
...
# -----------------------------------------------------------------------------
install_acme_tool() {
    run_local_script "ax-acme.sh" "ACME 证书管理脚本" "$AX_ACME_URL"
}
