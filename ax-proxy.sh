...
AX_ACME_SCRIPT="${AX_ACME_URL##*/}"      # 从 URL 提取: ax-acme.sh
CFWARP_SCRIPT="${CFWARP_URL##*/}"        # 从 URL 提取: CFwarp.sh
AX_OPTZ_SCRIPT="${AX_OPTZ_URL##*/}"      # 从 URL 提取: ax-optz.sh
...
{
    "listen": "__LISTEN__", "target": "__TARGET__", "key": "__KEY__", "crypt": "__CRYPT__", "mode": "__MODE__", "smuxver": 2, "mtu": 1200, "sndwnd": 512, "rcvwnd": 2048, "datashard": 10, "parityshard": 3, "dscp": 46, "nocomp": __NOCOMP__, "acknodelay": false, "nodelay": 1, "interval": 20, "resend": 2, "nc": 1, "sockbuf": 16777217, "smuxbuf": 16777217, "streambuf": 4194304, "keepalive": 5, "autoexpire": 600, "quiet": false, "tcp": __TCP__
}
EOM
...
collect_kcptun_params() {
    ...
    read -p "nocomp [true/false] (默认 true): " nocomp_input >&2
    nocomp_input=${nocomp_input:-true}
