...
# SSH命令：wget --no-check-certificate -O ax.sh "https://raw.githubusercontent.com/halibotee/scripts/main/ax.sh" && chmod +x ax.sh && ./ax.sh
...
WARP_GEOSITE_LIST_JSON='"geosite:google","geosite:openai","geosite:perplexity"'# 只能是geosite格式，不能有域名如 “ip-api.com”否则只会解析域名
...
create_new_xray_instance() {
    ...
    read -p "是否启用 WARP SOCKS5 分流 (默认禁用)？[y/N]: " enable_warp
    if [[ "$enable_warp" == "y" || "$enable_warp" == "Y" ]]; then
        local warp_addr="$DEFAULT_WARP_SOCKS5_ADDR" # [新] 自动使用默认地址
        read -p "请输入 WARP SOCKS5 端口 (默认: $DEFAULT_WARP_SOCKS_PORT): " warp_port; warp_port=${warp_port:-$DEFAULT_WARP_SOCKS_PORT}
...
# 日志函数 (假设已存在)
log() {
    # 检查 green 和 red 函数是否存在，如果不存在则定义简化的版本
    ...
    if ! declare -f red > /dev/null; then
     ...
     if ! declare -f yellow > /dev/null; then
        yellow(){ echo -e "\033[0;33m$1\033[0m"; }
    fi

    # [MODIFIED] 简化日志输出，使其与脚本其他部分一致
    echo -e "[$(date '+%H:%M:%S')] $(bold "$1")"
}


# --- [MODIFIED] 移除旧的 install_x_ui_yg, install_bbr_ylx2016, install_warp_fscarmen 函数 ---

# [NEW] 安装 VPS 一键优化脚本
install_sys_opt() {
    echo "----------------------------------------------------------------"
    log "即将开始下载并运行 VPS 一键优化脚本 (sysOpt.sh)..."
    
    # 运行用户提供的命令
    wget --no-check-certificate -O sysOpt.sh "https://raw.githubusercontent.com/halibotee/scripts/main/sysOpt.sh" && chmod +x sysOpt.sh && ./sysOpt.sh
    
    # 检查脚本是否成功下载（作为执行后的一个简单反馈）
    if [ ! -f "sysOpt.sh" ]; then
         echo "----------------------------------------------------------------"
         red "错误：下载 sysOpt.sh 失败或脚本未正确执行。"
         log "即将返回主菜单..."
         return 1 # 返回错误状态
    fi

    echo "----------------------------------------------------------------"
    green "VPS 一键优化脚本 (sysOpt.sh) 执行完毕。"
    log "即将返回主菜单..."
}
...
install_warp_yg() {
    echo "----------------------------------------------------------------"
    log "即将开始下载并运行 warp-yg 脚本 (CFwarp.sh)..."
    yellow "提示：请在接下来的脚本中选择 '安装Socks5-WARP' 选项。"
    sleep 2 # 给用户时间阅读提示
    ...
    if [ ! -f "CFwarp.sh" ]; then
         echo "----------------------------------------------------------------"
         red "错误：下载 CFwarp.sh 失败或脚本未正确执行。"
         log "即将返回主菜单..."
         return 1 # 返回错误状态
    fi
    
    echo "----------------------------------------------------------------"
    green "warp-yg 脚本 (CFwarp.sh) 执行完毕。"
    log "即将返回主菜单..."
}
...
main_menu(){
    ...
    while true; do
        ...
        cyan "--- 工具管理 ---"
        # [MODIFIED] 更改菜单项
        echo " 13) 安装 VPS一键优化脚本（含BBR+FQ）"
        echo " 14) 安装 warp-yg脚本（选择 安装Socks5-WARP）"
        echo "----------------------------------"   
        echo " 99) 卸载"
        echo " 0) 退出"
        echo "----------------------------------"
