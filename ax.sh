#!/bin/bash
# Vless、Hysteria2, UDP2RAW & KCPTUN 实例管理脚本 ，warp_socket5分流
# 首尾必须包括rm -f ax.sh && cat << 'EOF' > ax.sh ，EOF chmod +x ax.sh && ./ax.sh
...
# Xray Shadowsocks (TCP/UDP) 模板 (用于 KCPTUN / UDP2RAW 串联)
...
view_all_configs() {
    ...
    local standalone_hy2=$(get_standalone_instances "hysteria2"); if [[ -n "$standalone_hy2" ]]; then echo; cyan "--- Hysteria2 (独立) 实例订阅链接 ---"; echo 
    for id in $standalone_hy2; do green "$(generate_hy2_subscription_link $id)"; done; echo "----------------------------------"; fi
    
    local standalone_reality=$(get_standalone_instances "xray_reality"); if [[ -n "$standalone_reality" ]]; then echo; cyan "--- VLESS+Reality (独立) 实例订阅链接 ---"; echo 
    for id in $standalone_reality; do green "$(generate_xray_reality_link $id)"; done; echo "----------------------------------"; fi
    local standalone_mkcp=$(get_standalone_instances "xray_mkcp"); if [[ -n "$standalone_mkcp" ]]; then echo; cyan "--- VLESS+mKCP (独立) 实例订阅链接 ---"; echo 
    for id in $standalone_mkcp; do green "$(generate_xray_mkcp_link $id)"; done; echo "----------------------------------"; fi
    local standalone_ss=$(get_standalone_instances "xray_ss"); if [[ -n "$standalone_ss" ]]; then echo; cyan "--- Shadowsocks (独立) 实例订阅链接 ---"; echo
    for id in $standalone_ss; do green "$(generate_xray_ss_link $id)"; done; echo "----------------------------------"; fi
    
    local standalone_udp2raw=$(get_standalone_instances "udp2raw"); if [[ -n "$standalone_udp2raw" ]]; then echo; for id in $standalone_udp2raw; do view_udp2raw_client_config "$id"; echo; done; echo "----------------------------------"; fi
    local standalone_kcptun=$(get_standalone_instances "kcptun"); if [[ -n "$standalone_kcptun" ]]; then echo; for id in $standalone_kcptun; do view_kcptun_client_config "$id"; echo; done; fi
...
uninstall_all(){
    read -p "确认要卸载吗？[y/N]: " confirm; if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then yellow "操作已取消。"; return 1; fi
    ...
    rm -f /etc/systemd/system/kcptun@.service /etc/systemd/system/udp2raw@.service /etc/systemd/system/hysteria2@.service /etc/systemd/system/${XRAY_SERVICE_NAME}@.service /etc/systemd/system/hysteria-server.service /etc/systemd/system/hysteria-server@.service
    green "Systemd 服务文件已删除。"
    if [[ "$nuke_choice" == "y" || "$nuke_choice" == "Y" ]]; then
        log "步骤 3: 执行彻底清理..."
        rm -rf /etc/kcptun-udp2raw /etc/hysteria2 /etc/xray /etc/hysteria
        green "程序和配置文件目录已删除。"
        log "步骤 4: 删除生成的证书..."
