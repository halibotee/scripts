...
package_plugin(){
	...
	local PKG_VERSION=$(cat /koolshare/merlinclash/version 2>/dev/null)
	PKG_VERSION=${PKG_VERSION:-1.2.1_chian_1.0}
	local PKG_NAME="MC2_${PKG_VERSION}_ARM64.tar.gz"
	rm -rf "$PKG_DIR" /tmp/mc_package.tar.gz /tmp/upload/mc_package.tar.gz "/tmp/upload/${PKG_NAME}"
	...
	cd /tmp && tar -czf "/tmp/${PKG_NAME}" -C "$PKG_DIR" merlinclash
	if [ -s "/tmp/${PKG_NAME}" ]; then
		echo_date "压缩完成，导出 ${PKG_NAME} ..." >> $LOG_FILE
		mv -f "/tmp/${PKG_NAME}" "/tmp/upload/${PKG_NAME}"
		echo "${PKG_NAME}" > /tmp/upload/mc_pkg_name.txt
		rm -rf "$PKG_DIR"
	else
		echo_date "打包失败，内容为空！" >> $LOG_FILE
		rm -rf "$PKG_DIR" "/tmp/${PKG_NAME}"
		echo BBABBBBC >> $LOG_FILE
		exit 1
	fi
}
