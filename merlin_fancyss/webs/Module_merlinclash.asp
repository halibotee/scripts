	...
	//------------------------------------打包插件 BEGIN--------------------------------------------
	var _pkg_downloaded = false;
	function pkg_do_download() {
		if (_pkg_downloaded) return;
		_pkg_downloaded = true;
		$.get("/_temp/mc_pkg_name.txt", function(name) {
			name = name.trim() || "mc_package.tar.gz";
			var a = document.createElement('a');
			a.href = "http://" + window.location.hostname + "/_temp/" + name;
			a.download = name;
			a.click();
			window.URL.revokeObjectURL(a.href);
		}).fail(function() {
			var a = document.createElement('a');
			a.href = "http://" + window.location.hostname + "/_temp/mc_package.tar.gz";
			a.download = "mc_package.tar.gz";
			a.click();
			window.URL.revokeObjectURL(a.href);
		});
	}
	function get_package_log() {
		$.ajax({
			...
			success: function(response) {
				...
				retArea.value = response.replace("BBABBBBC", " ");
				retArea.scrollTop = retArea.scrollHeight;
				setTimeout("get_package_log();", 500);
			},
			error: function() {
				setTimeout("get_package_log();", 500);
			}
	...
	function package_clashdata() {
		layer.confirm('<li>将打包当前所有文件为可安装的MC2插件包，继续吗？</li>', {
			shade: 0.8,
		}, function(index) {
			_pkg_downloaded = false;
			$("#log_content3").attr("rows", "20");
			db_merlinclash["merlinclash_action"] = 44;
			var id = parseInt(Math.random() * 100000000);
			var postData = {"id": id, "method": "clash_backup.sh", "params":["package"], "fields": {}};
			showMCLoadingBar(60);
			get_package_log();
			intoQueue({
				type: "POST",
				...
				success: function(response) {
					if (response.result == id) {
						setTimeout(pkg_do_download, 2000);
					}
				}
			});
			layer.close(index);
									...
									<tr>
										<td bgcolor="#4D595D" colspan="3" valign="top">
											...
											<div class="SimpleNote" id="head_illustrate"><i></i>
												<p>Magic Catling2是一个基于Mihomo内核的代理程序，支持SS、SSR、Vmess、Vless、Trojan、Hysteria、Chian串联等协议科学上网。</p>
												<p>&nbsp;</p>
											</div>
																			...
																			<table style="margin:10px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable" id="merlinclash_switch_table">
																				...
																				<tr>
																					...
																					<td colspan="2">
																						...
																						<a type="button" style="vertical-align: middle; cursor:pointer;" id="clashdata-btn-package" class="ks_btn" onclick="package_clashdata()" >打包插件</a>
																					</td>
																				</tr>
																				<tr>
