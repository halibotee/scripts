#!/bin/bash
# VPS 激进优化脚本 - 集成网络配置激进方案
# 基于 vps_optimizert.sh 和 tcp.sh 菜单27
...
fn_setup_aggressive_network() {
    ...
    if [ $FORCE_YES -eq 1 ]; then
        ...
        echo ""
        echo "示例1 (完整参数): 180 500 500"
        echo "示例2 (单个参数): 200  # 结果: 延迟=200ms, 其他使用默认值"
        echo ""
        ...
        if [ -z "$input" ]; then
            ...
            read -r latency local_bw vps_bw <<<\"$input\"
    ...
    fn_log "信息" "创建激进网络配置: $config_file"
...
EOF
    ...
    fn_log "成功" "激进网络配置文件已创建: $config_file"
...
fn_show_status_report() {
    ...
    fn_print_line "激进网络配置" "$sysctl_status" "[ 已配置 ]" "[ 未配置 ]" ""
