...
register_warp_wireguard() {
    ...
    # 从 client_id 解码 reserved bytes
    local reserved="[0, 0, 0]"
    if command -v python3 &>/dev/null && [[ -n "$client_id" && "$client_id" != "null" ]]; then
        reserved=$(python3 -c "
import base64, json
cid = '$client_id'
pad = 4 - len(cid) % 4
if pad != 4: cid += '=' * pad
decoded = base64.b64decode(cid)
print(json.dumps(list(decoded[:3])))
" 2>/dev/null)
...
test_warp_connectivity() {
    ...
    local test_domains=("$@")
    [[ ${#test_domains[@]} -le 1 ]] && test_domains=("chatgpt.com" "www.gstatic.com")
    sleep "$wait_seconds"
    for domain in "${test_domains[@]:1}"; do
        local code=$(curl -s --max-time 5 --socks5-hostname 127.0.0.1:17888 -o /dev/null -w "%{http_code}" "https://$domain" 2>/dev/null)
        if [[ "$code" != "000" ]]; then
            green "  WARP 连通性测试通过: $domain → HTTP $code"
            return 0
        fi
    done
    yellow "  WARP 连通性测试失败 (所有测试域名均返回 000)"
    return 1
}
...
warp_ip_optimize() {
    ...
    local old_ip=$(curl -s --max-time 3 --socks5-hostname 127.0.0.1:17888 http://ip-api.com/json/ 2>/dev/null | jq -r '.query // "未知"')
    echo "当前出口IP: $old_ip"
    ...
    if $success; then
        local new_ip=$(curl -s --max-time 3 --socks5-hostname 127.0.0.1:17888 http://ip-api.com/json/ 2>/dev/null | jq -r '.query // "未知"')
        green "出口IP更换成功: $old_ip → $new_ip"
        rm -f "$backup_file"
        sync; return 0
    fi
    red "IP优选失败, 恢复原配置..."
