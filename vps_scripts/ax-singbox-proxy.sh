...
cfg['route']['rules'] = rules
    if '$final_outbound':
        ...
        cfg['route']['rule_set'] = [
            {"type": "local", "tag": "geoip-cn", "format": "binary", "path": "$SINGBOX_INSTALL_DIR/geoip.srs"},
            {"type": "local", "tag": "geosite-cn", "format": "binary", "path": "$SINGBOX_INSTALL_DIR/geosite.srs"}
        ]
...
download_geoip_dbs() {
    local need_dl=false
    if [[ ! -f "$SINGBOX_INSTALL_DIR/geoip.srs" ]]; then need_dl=true; fi
    if [[ ! -f "$SINGBOX_INSTALL_DIR/geosite.srs" ]]; then need_dl=true; fi
    $need_dl || { green "✓ geoip/geosite 规则集已存在"; return 0; }

    log "下载 geoip 规则集..."
    download_with_retry "https://github.com/SagerNet/sing-geoip/releases/latest/download/geoip.srs" "$SINGBOX_INSTALL_DIR/geoip.srs" || {
        red "✗ geoip.srs 下载失败"; return 1
    }
    log "下载 geosite 规则集..."
    download_with_retry "https://github.com/SagerNet/sing-geosite/releases/latest/download/geosite.srs" "$SINGBOX_INSTALL_DIR/geosite.srs" || {
        red "✗ geosite.srs 下载失败"; return 1
    }
    green "✓ geoip/geosite 规则集下载完成"
}
