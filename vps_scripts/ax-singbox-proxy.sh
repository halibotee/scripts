...
# 原生 IP 解锁则 direct, 否则 warp-ep; 自定义路由走 08_custom_route.json
...
register_warp_wireguard() {
    ...
    # 先用 fscarmen 共享账户 (reserved 非零, CF 已拒绝 [0,0,0])
...
EOF
    ...
    green "✓ WARP 使用 fscarmen 共享账户已保存至 $warp_key_file"
    green "  Reserved: [78, 135, 76]"
    
    # 尽力尝试 API 注册, 成功时覆盖共享账户 (此时共享账户已写入, 失败不阻塞)
    ...
    if [[ -n "$wg_private" && -n "$wg_public" ]]; then
        ...
        local reg_id=$(echo "$api_result" | jq -r '.id // empty' 2>/dev/null)
        if [[ -n "$reg_id" && "$reg_id" != "null" ]]; then
            local cid=$(echo "$api_result" | jq -r '.config.client_id // .client_id // empty' 2>/dev/null)
            ...
            [[ -z "$wg_address_v4" || "$wg_address_v4" == "null" ]] && wg_address_v4="172.16.0.2"
            [[ -z "$wg_address_v6" || "$wg_address_v6" == "null" ]] && wg_address_v6=""
            [[ -z "$wg_private_key" || "$wg_private_key" == "null" ]] && wg_private_key="$wg_private"
...
EOF
            chmod 600 "$warp_key_file"
            green "✓ WARP API 注册成功! 已覆盖共享账户至 $warp_key_file"
        fi
    fi
}
...
# 自动合并 08_custom_route.json 的自定义 warp-ep 规则 (对齐 fscarmen)
# 写入成功返回 0, 失败返回 1
# -----------------------------------------------------------------------------
apply_warp_config_to_file() {
    local wg_ep=$1 route_rules=$2 config_file="$SINGBOX_INSTALL_DIR/singbox.json"
    local custom_file="$SINGBOX_INSTALL_DIR/08_custom_route.json"
    local custom_tmp=""
    ...
    if [[ -f "$custom_file" ]]; then
        custom_tmp=$(umask 077 && mktemp) || { rm -f "$tmpfile" "$ep_tmp" "$rules_tmp"; return 1; }
        printf '%s\n' "$(cat "$custom_file")" > "$custom_tmp"
    else
        custom_tmp=$(umask 077 && mktemp) || { rm -f "$tmpfile" "$ep_tmp" "$rules_tmp"; return 1; }
        echo '{"route":{"rule_set":[],"rules":[]}}' > "$custom_tmp"
    fi
    # 合并规则 (geosite-openai + 自定义 rule_set 定义, 基础规则 + 自定义规则)
    if ! python3 - "$ep_tmp" "$rules_tmp" "$custom_tmp" "$config_file" "$tmpfile" "$SINGBOX_INSTALL_DIR" <<'PYEOF' 2>/dev/null
import json, sys
ep_tmp, rules_tmp, custom_tmp, config_file, tmpfile, sdir = sys.argv[1:]
cfg = json.load(open(config_file))
cfg['endpoints'] = json.load(open(ep_tmp))
base = json.load(open(rules_tmp))
custom = json.load(open(custom_tmp))
ruleset = [
    {'tag': 'geosite-openai', 'type': 'remote', 'format': 'binary',
     'url': 'https://raw.githubusercontent.com/SagerNet/sing-geosite/rule-set/geosite-openai.srs'}
]
ruleset += custom.get('route', {}).get('rule_set', [])
cfg['route']['rules'] = base + custom.get('route', {}).get('rules', [])
...
PYEOF
    then
        rm -f "$tmpfile" "$ep_tmp" "$rules_tmp" "$custom_tmp"; return 1
    fi
    rm -f "$ep_tmp" "$rules_tmp" "$custom_tmp"
...
check_chatgpt() {
    ...
    local r1
    r1=$(curl -s --max-time 6 $stack -A "$ua" \
    ...
    local r2
    r2=$(curl -s --max-time 6 $stack -A "$ua" "https://ios.chat.openai.com/" 2>/dev/null)
...
sync_warp_config() {
    ...
    wg_ep=$(apply_warp_wireguard_config) || return 1
    route_rules=$SINGBOX_WARP_ROUTE_RULES
    route_rules=${route_rules/__CHATGPT_OUT__/$(get_chatgpt_out)}
...
# ===================== 自定义 warp-ep 路由规则 (对齐 fscarmen) =====================

# 统计自定义路由规则数量
custom_route_count() {
    local custom_file="$SINGBOX_INSTALL_DIR/08_custom_route.json"
    if [ -s "$custom_file" ]; then
        jq '[.route.rules[]? | ((.domain_suffix // []) | length) + ((.rule_set // []) | length)] | add // 0' "$custom_file" 2>/dev/null || echo 0
    else
        echo 0
    fi
}

# 将 warp-ep 的 domain_suffix / rule_set 合并到同一条 route rule
custom_route_compact_rules() {
    local custom_file="$SINGBOX_INSTALL_DIR/08_custom_route.json"
    local tmp="${custom_file}.tmp"
    [ -s "$custom_file" ] || return 1
    if ! jq '
        ([.route.rules[]? | select((.outbound // "warp-ep") == "warp-ep") | .domain_suffix // [] | .[]] | unique) as $domains |
        ([.route.rules[]? | select((.outbound // "warp-ep") == "warp-ep") | .rule_set // [] | .[]] | unique) as $sets |
        ([.route.rules[]? | select((.outbound // "warp-ep") != "warp-ep")]) as $others |
        .route.rules = ($others +
    ...
    ' "$custom_file" > "$tmp" 2>/dev/null; then
        rm -f "$tmp"; return 1
    fi
    mv "$tmp" "$custom_file"
}

# 逗号分隔输入转数组 (支持半角/全角逗号、顿号、分号、竖线)
custom_route_csv_to_array() {
    ...
    mapfile -t out_array < <(
        printf '%s\n' "$input_csv" |
            sed 's/\x1b\[[0-9;?]*[A-Za-z]//g; s/[，、；;|]/,/g; s/[[:space:]]//g; s/,/\n/g; /^$/d'
    )
}

# 校验 rule_set 是否存在, 返回下载 URL (SagerNet 优先, MetaCubeX 兜底)
check_rule_set_exists() {
    local name=$1 srs="${1}.srs" sag metacubex match
    sag=$(curl -sL --connect-timeout 5 --max-time 15 "https://api.github.com/repos/SagerNet/sing-geosite/git/trees/rule-set?recursive=1" 2>/dev/null)
    if echo "$sag" | jq -e ".tree[]? | select(.path == \"$srs\")" >/dev/null 2>&1; then
        echo "https://raw.githubusercontent.com/SagerNet/sing-geosite/rule-set/${srs}"; return 0
    fi
    metacubex=$(curl -sL --connect-timeout 5 --max-time 15 "https://api.github.com/repos/MetaCubeX/meta-rules-dat/git/trees/sing?recursive=1" 2>/dev/null)
    match=$(echo "$metacubex" | jq -r "[.tree[]? | select(.path | endswith(\"/${srs}\") or . == \"${srs}\") | .path] | first // empty" 2>/dev/null)
    if [ -n "$match" ]; then
        echo "https://raw.githubusercontent.com/MetaCubeX/meta-rules-dat/sing/${match}"; return 0
    fi
    if [ -z "$sag" ] && [ -z "$metacubex" ]; then
        warning "GitHub API 不可用, 降级使用默认 URL (可能无效)" >&2
        echo "https://raw.githubusercontent.com/SagerNet/sing-geosite/rule-set/${srs}"; return 0
    fi
    return 1
}

# 添加自定义路由规则 (固定 warp-ep 出站)
custom_route_add() {
    local custom_file="$SINGBOX_INSTALL_DIR/08_custom_route.json"
    echo "选择规则类型:"
    echo " 1. domain_suffix (域名后缀)"
    echo " 2. rule_set (规则集)"
    read -p "请选择: " rule_type_choice
    ...
    local validated_values=() rule_set_urls=()
    if [ "$rule_type" = "domain_suffix" ]; then
        read -p "输入域名后缀 (逗号分隔, 如 google.com,telegram.org): " domain_input
        [ -z "$domain_input" ] && { red "不能为空"; return; }
        local domains=() d
        custom_route_csv_to_array "$domain_input" domains
        for d in "${domains[@]}"; do
            d=$(sed 's/。/./g' <<< "${d,,}")
            [[ "$d" =~ ^[a-z0-9]([a-z0-9.-]*[a-z0-9])?\.[a-z]{2,}$ ]] && validated_values+=("$d") || yellow "无效域名: $d"
        done
    else
        read -p "输入规则集名称 (逗号分隔, 如 geosite-google,geosite-telegram): " ruleset_input
        [ -z "$ruleset_input" ] && { red "不能为空"; return; }
        local rulesets=() rname url
        custom_route_csv_to_array "$ruleset_input" rulesets
        for rname in "${rulesets[@]}"; do
            rname="${rname,,}"
            rname=$(sed -E 's#^.*/##; s/\.srs$//I' <<< "$rname")
            [[ -n "$rname" && ! "$rname" =~ ^geo(site|ip)- ]] && rname="geosite-${rname}"
            [ -z "$rname" ] && continue
            url=$(check_rule_set_exists "$rname") || { yellow "规则集 $rname 未找到, 跳过"; continue; }
            validated_values+=("$rname"); rule_set_urls+=("$url")
        done
    fi
    [ "${#validated_values[@]}" -eq 0 ] && { red "无有效输入"; return; }

    [ -s "$custom_file" ] || echo '{"route":{"rule_set":[],"rules":[]}}' > "$custom_file"
    local tmp="${custom_file}.tmp"

    if [ "$rule_type" = "domain_suffix" ]; then
        local domains_json
        domains_json=$(printf '%s\n' "${validated_values[@]}" | jq -R . | jq -s 'unique')
        if ! jq --argjson domains "$domains_json" --arg out "warp-ep" '
            ([.route.rules[]? | select((.outbound // "warp-ep") == "warp-ep") | .domain_suffix // [] | .[]] | unique) as $old_domains |
            ([.route.rules[]? | select((.outbound // "warp-ep") != "warp-ep")]) as $others |
            .route.rules = $others + [{domain_suffix: ($old_domains + $domains | unique), action: "route", outbound: $out}]
        ' "$custom_file" > "$tmp" 2>/dev/null; then
            rm -f "$tmp"; red "添加失败"; return 1
        fi
        mv "$tmp" "$custom_file"
    else
        for i in "${!validated_values[@]}"; do
            if ! jq --arg tag "${validated_values[$i]}" --arg url "${rule_set_urls[$i]}" '
                .route.rule_set = (.route.rule_set // []) |
                .route.rule_set |= if any(.[]; .tag == $tag) then . else . + [{tag: $tag, type: "remote", format: "binary", url: $url}] end
            ' "$custom_file" > "$tmp" 2>/dev/null; then
                rm -f "$tmp"; red "添加失败"; return 1
            fi
            mv "$tmp" "$custom_file"
        ...
        local names_json
        names_json=$(printf '%s\n' "${validated_values[@]}" | jq -R . | jq -s 'unique')
        if ! jq --argjson names "$names_json" --arg out "warp-ep" '
            ([.route.rules[]? | select((.outbound // "warp-ep") == "warp-ep") | .rule_set // [] | .[]] | unique) as $old_sets |
            ([.route.rules[]? | select((.outbound // "warp-ep") != "warp-ep")]) as $others |
            .route.rules = $others + [{rule_set: ($old_sets + $names | unique), action: "route", outbound: $out}]
        ' "$custom_file" > "$tmp" 2>/dev/null; then
            rm -f "$tmp"; red "添加失败"; return 1
        fi
        mv "$tmp" "$custom_file"
...
# 展开后的自定义路由项 (每个 domain_suffix / rule_set 元素单独一行)
...
custom_route_view() {
    local custom_file="$SINGBOX_INSTALL_DIR/08_custom_route.json"
    [ -s "$custom_file" ] || { yellow "未配置自定义路由规则。"; return 1; }
    local items=()
    mapfile -t items < <(custom_route_items_json)
    [ "${#items[@]}" -eq 0 ] && { yellow "未配置自定义路由规则。"; return 1; }
    echo "  #   Type           Match"
    echo "  ---  --------------  --------"
    local i=0 item type match
...
custom_route_delete() {
    ...
    local items=()
    mapfile -t items < <(custom_route_items_json)
    read -p "输入要删除的规则编号 (逗号分隔): " del_input
    [ -z "$del_input" ] && return
    local del_nums=() num
    custom_route_csv_to_array "$del_input" del_nums
    local picks=()
    for num in "${del_nums[@]}"; do
        num=$(sed 's/[^0-9]//g' <<< "$num")
        [[ "$num" =~ ^[0-9]+$ && $num -ge 1 && $num -le "${#items[@]}" ]] && picks+=("${items[$((num - 1))]}")
    done
    [ "${#picks[@]}" -eq 0 ] && { red "无效编号"; return; }
    local del_json tmp
    del_json=$(printf '%s\n' "${picks[@]}" | jq -s 'unique_by(.rule_index, .type, .match)')
    tmp="${custom_file}.tmp"
    if ! jq --argjson del "$del_json" '
        .route.rules |= (
          [.[]?] as $rules |
          reduce range(0; ($rules | length)) as $idx ([];
            ($rules[$idx]) as $rule |
            ($del | map(select(.rule_index == $idx and .type == "domain_suffix") | .match)) as $rm_d |
            ($del | map(select(.rule_index == $idx and .type == "rule_set") | .match)) as $rm_s |
            ($rule
              | if .domain_suffix? != null then .domain_suffix = [.domain_suffix[]? | select(($rm_d | index(.)) == null)] else . end
              | if .rule_set? != null then .rule_set = [.rule_set[]? | select(($rm_s | index(.)) == null)] else . end
              | if ((.domain_suffix // []) | length) == 0 then del(.domain_suffix) else . end
              | if ((.rule_set // []) | length) == 0 then del(.rule_set) else . end
            ) as $nr |
            if (($nr.domain_suffix? != null) or ($nr.rule_set? != null)) then . + [$nr] else . end
          )
        )
    ' "$custom_file" > "$tmp" 2>/dev/null; then
        rm -f "$tmp"; red "删除失败"; return 1
    fi
    mv "$tmp" "$custom_file"
    custom_route_compact_rules
# 清理孤立的 rule_set 定义 (只保留仍被引用的)
    jq '
        ([.route.rules[]? | .rule_set // [] | .[]] | unique) as $used |
        .route.rule_set |= [.[]? | select(.tag as $t | $used | index($t))]
    ' "$custom_file" > "$tmp" 2>/dev/null && mv "$tmp" "$custom_file"
    if [ "$(jq '.route.rules | length // 0' "$custom_file" 2>/dev/null)" = "0" ]; then
        rm -f "$custom_file"
    fi
    green "✓ 自定义路由规则已删除"
    sync_warp_config
...
# 自定义 warp-ep 出站路由规则子菜单
custom_route_menu() {
    while true; do
        echo "------ 自定义 warp-ep 出站路由规则 (规则数: $(custom_route_count)) ------"
        echo " 1. 添加规则"
        echo " 2. 查看规则"
        echo " 3. 删除规则"
        echo " 0. 返回"
...
change_warp_account_apply() {
    ...
    route_rules=${route_rules/__CHATGPT_OUT__/$(get_chatgpt_out)}
    apply_warp_config_to_file "$wg_ep" "$route_rules" || return 1
    if systemctl reload ax-singbox.service 2>/dev/null && systemctl is-active --quiet ax-singbox.service; then
        green "✓ WARP 账户已更新, Sing-box 已热重载"
        return 0
    else
        red "Sing-box 重载失败, 请检查日志" >&2
        return 1
    fi
}
...
warp_management_menu() {
    while true; do
        ...
        echo " 2) 自定义 warp-ep 出站路由规则 (规则数: $(custom_route_count))"
        echo " 3) 更换 WARP 账户"
        echo " 0) 返回"
        ...
        case "$warp_choice" in
            1)
                if $cfg_enabled; then
                    if disable_warp_in_config; then sync; systemctl reload ax-singbox.service 2>/dev/null; green "WARP 已禁用。"; else yellow "禁用失败。"; fi
                else
...
import json, sys
cfg = json.load(open('$config_file'))
cfg.setdefault('log', {})['level'] = 'error'
...
ntp.setdefault('server', 'time.apple.com')
...
create_initial_singbox_config() {
    ...
    if [[ -f "$SINGBOX_INSTALL_DIR/.warp_wireguard.json" ]]; then
        if ! local wg_ep=$(apply_warp_wireguard_config); then
            ...
            route_rules=$SINGBOX_WARP_ROUTE_RULES
            route_rules=${route_rules/__CHATGPT_OUT__/$(get_chatgpt_out)}
            route_rule_set=$SINGBOX_GEOSITE_OPENAI_RULESET
        fi
    fi
...
view_kcptun_client_config(){
    ...
    yellow "方法一: 使用 JSON 配置文件 (推荐)"; yellow "将以下内容保存为客户端的 config.json (本地监听地址 ${CLIENT_KCPTUN_LISTEN_ADDR} 可自行修改):"; green "$(jq --arg listen "${CLIENT_KCPTUN_LISTEN_ADDR}" --arg target "${ip}:${server_port}" --argjson sndwnd "$server_rcvwnd" --argjson rcvwnd "$server_sndwnd" --argjson mtu 1300 '.listen = $listen | .target = $target | .sndwnd = $sndwnd | .rcvwnd = $rcvwnd | .mtu = $mtu' "$conf")"; echo
...
import_subscription_links() {
    ...
    if [[ "$full_link" == CHAIN://* ]]; then
        ...
        if [[ "$p1" == ss://* && "$p2" == kcptun://* && "$p3" == udp2raw://* ]]; then
            ...
            local ss_raw="${p1#ss://}"
            local ss_b64="${ss_raw%%@*}"
            local pad=$((4 - ${#ss_b64} % 4))
            [[ $pad -lt 4 ]] && printf -v ss_b64 '%s%*s' "$ss_b64" $pad | tr ' ' '='
