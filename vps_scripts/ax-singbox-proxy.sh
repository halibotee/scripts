...
EOF
    ...
    while IFS= read -r line; do
        ...
        line="${line//[[:space:]]/}"
        [[ -z "$line" ]] && continue
        local http_code=$(curl -s --max-time 3 -o /dev/null -w "%{http_code}" "https://${line}" 2>/dev/null)
        if [[ "$http_code" == "200" || "$http_code" == "204" || "$http_code" == "301" || "$http_code" == "302" || "$http_code" == "304" || "$http_code" == "307" || "$http_code" == "308" ]]; then
            green "  ${line} → 可达 ✓ (跳过WARP)" >&2
            continue
        fi
        yellow "  ${line} → 不可达 ✗ (走WARP)" >&2
        if $first; then
            echo -n "\"${line}\""
            first=false
        else
