...
WARP_MODE_FILE="$SINGBOX_INSTALL_DIR/.warp_mode"  # "custom" 或 "global"
...
# -----------------------------------------------------------------------------
# Sing-box 路由规则块 (JSON) - 全局 WARP 分流 (geoip:cn/geosite:cn -> direct, rest -> warp-ep)
# -----------------------------------------------------------------------------
read -r -d '' SINGBOX_GLOBAL_WARP_ROUTE_RULES <<'EOM'
[
    { "outbound": "direct", "ip_is_private": true },
...
register_warp_wireguard() {
    ...
    # 注册失败时回退 fscarmen 共享账户 (reserved 非零, CF 已拒绝 [0,0,0] 共享账号)
    local reg_id=$(echo "$api_result" | jq -r '.id // empty' 2>/dev/null)
    if [[ -z "$reg_id" || "$reg_id" == "null" || -z "$api_result" ]]; then
        yellow "警告: Cloudflare WARP API 注册失败, 回退 fscarmen 共享账户。" >&2
...
EOF
        ...
        green "✓ WARP 回退共享账户已保存至 $warp_key_file"
    ...
    # 解码 client_id -> reserved (3 字节); 失败则用 fscarmen 共享账号保留值
...
# -----------------------------------------------------------------------------
# 写入 WARP endpoint + 路由规则到 singbox.json (内部 helper)
# 参数: $1=endpoint JSON, $2=route_rules JSON, $3=final_outbound (可选，不传则移除 route.final)
# 写入成功返回 0, 失败返回 1
# -----------------------------------------------------------------------------
apply_warp_config_to_file() {
    local wg_ep=$1 route_rules=$2 final_outbound=${3:-} config_file="$SINGBOX_INSTALL_DIR/singbox.json"
...
if '$final_outbound':
    cfg['route']['final'] = '$final_outbound'
    cfg['route']['rule_set'] = [
        {'type': 'local', 'tag': 'geoip-cn', 'format': 'binary', 'path': '$SINGBOX_INSTALL_DIR/geoip-cn.srs'},
        {'type': 'local', 'tag': 'geosite-cn', 'format': 'binary', 'path': '$SINGBOX_INSTALL_DIR/geosite-cn.srs'}
    ]
elif 'final' in cfg.get('route', {}):
    del cfg['route']['final']
...
# 已启用时跳过 (幂等)
# 包含 IP 优选: 注册 → 测试 → 不通则换注册, 最多 MAX_RETRIES 次
# -----------------------------------------------------------------------------
...
sys.exit(1)
" 2>/dev/null; then
        if [[ "$(get_warp_mode)" != "global" ]]; then
            green "WARP 已在配置中启用，无需重复操作。"; return 0
        fi
        yellow "当前 WARP 已启用 (全局模式)，正在切换至自定义模式..."
        disable_warp_in_config || return 1
    fi
    local warp_key_file="$SINGBOX_INSTALL_DIR/.warp_wireguard.json"
    if [[ ! -f "$warp_key_file" ]]; then
        register_warp_wireguard || return 1
    fi
    local warp_domains; warp_domains=$(get_warp_domain_list)
    if [[ -z "$warp_domains" ]]; then
        yellow "所有分流域名直连可达，无需 WARP。"; return 0
    fi
    ...
    while [[ $attempt -lt $max_retries ]]; do
        ...
        route_rules=${route_rules/__WARP_DOMAIN_SUFFIX_LIST__/$warp_domains}
        if apply_warp_config_to_file "$wg_ep" "$route_rules"; then
            ...
            if test_warp_connectivity 5; then
                echo "custom" > "$WARP_MODE_FILE"
                green "✓ WARP 已启用 (IP优选完成, 尝试 $attempt 次)"
                return 0
            fi
        fi
        if [[ $attempt -lt $max_retries ]]; then
            ...
            register_warp_wireguard || return 1
            warp_domains=$(get_warp_domain_list)
            [[ -z "$warp_domains" ]] && { green "所有分流域名直连可达，无需 WARP。"; return 0; }
        fi
    done
    red "✗ WARP IP优选失败, 所有 $max_retries 次注册均无法连通。已回滚。"
...
# -----------------------------------------------------------------------------
# 启用全局 WARP 分流 (geoip:cn -> direct, 其余 -> warp-ep)
# 与 enable_warp_in_config 互斥，先禁用已有的再启用
# -----------------------------------------------------------------------------
enable_global_warp_in_config() {
    local config_file="$SINGBOX_INSTALL_DIR/singbox.json"
    if [[ ! -f "$config_file" ]]; then red "错误: singbox.json 不存在。" >&2; return 1; fi
    if ! command -v python3 &>/dev/null; then red "错误: python3 未安装。" >&2; return 1; fi
...
sys.exit(1)
" 2>/dev/null; then
        local cur_mode=$(get_warp_mode)
        if [[ "$cur_mode" == "global" ]]; then
            green "全局 WARP 分流已启用，无需重复操作。"; return 0
        fi
        yellow "当前 WARP 已启用 (自定义模式)，正在切换至全局模式..."
        disable_warp_in_config || return 1
    fi

    # 下载 geoip/geosite 数据库
    download_geoip_dbs || return 1
    ...
    local max_retries=5 attempt=0 wg_ep
    while [[ $attempt -lt $max_retries ]]; do
        attempt=$((attempt + 1))
        wg_ep=$(apply_warp_wireguard_config) || return 1
        # 全局模式: route rules 不含分流域名，设置 route.final = warp-ep
        if apply_warp_config_to_file "$wg_ep" "$SINGBOX_GLOBAL_WARP_ROUTE_RULES" "warp-ep"; then
            systemctl restart ax-singbox.service 2>/dev/null
            if test_warp_connectivity 10; then
                echo "global" > "$WARP_MODE_FILE"
                green "✓ 全局 WARP 分流已启用 (IP优选完成, 尝试 $attempt 次)"
                return 0
            fi
        fi
        if [[ $attempt -lt $max_retries ]]; then
    ...
    red "✗ 全局 WARP 启用失败, 所有 $max_retries 次注册均无法连通。已回滚。"
    disable_warp_in_config
    systemctl restart ax-singbox.service 2>/dev/null
    return 1
}
...
" 2>/dev/null; then
    ...
    if [[ "$(get_warp_mode)" == "global" ]]; then
        yellow "当前为全局 WARP 分流模式，编辑分流域名不生效。请先切换至自定义分流模式。"; return 1
    fi
    local route_rules=$SINGBOX_DIRECT_ROUTE_RULES
    local warp_domains; warp_domains=$(get_warp_domain_list)
    if [[ -z "$warp_domains" ]]; then
        yellow "所有分流域名直连可达，无需 WARP 分流更新。"; return 0
    fi
    route_rules=${route_rules/__WARP_DOMAIN_SUFFIX_LIST__/$warp_domains}
    local tmpfile; tmpfile=$(umask 077 && mktemp) || { red "无法创建临时文件" >&2; return 1; }
...
# -----------------------------------------------------------------------------
# 获取当前 WARP 模式 (custom/global)，无文件时默认 custom
# -----------------------------------------------------------------------------
get_warp_mode() {
    if [[ -f "$WARP_MODE_FILE" ]]; then
        cat "$WARP_MODE_FILE"
    else
        echo "custom"
    fi
...
# -----------------------------------------------------------------------------
# 下载 geoip.db / geosite.db (全局 WARP 分流需要)
# -----------------------------------------------------------------------------
download_geoip_dbs() {
    local geoip_srs="$SINGBOX_INSTALL_DIR/geoip-cn.srs"
    local geosite_srs="$SINGBOX_INSTALL_DIR/geosite-cn.srs"
    if [[ -f "$geoip_srs" && -f "$geosite_srs" ]]; then
        green "✓ geoip/geosite 规则集已存在"; return 0
    fi

    log "下载 geoip.db..."
    download_with_retry "https://github.com/SagerNet/sing-geoip/releases/latest/download/geoip.db" "$SINGBOX_INSTALL_DIR/geoip.db" || {
        red "✗ geoip.db 下载失败"; return 1
    }
    log "下载 geosite.db..."
    download_with_retry "https://github.com/SagerNet/sing-geosite/releases/latest/download/geosite.db" "$SINGBOX_INSTALL_DIR/geosite.db" || {
        red "✗ geosite.db 下载失败"; return 1
    }

    log "编译 geoip-cn.srs..."
    "$SINGBOX_INSTALL_DIR/sing-box" geoip export cn \
        -f "$SINGBOX_INSTALL_DIR/geoip.db" \
        -o /tmp/geoip-cn.json 2>/dev/null && \
    "$SINGBOX_INSTALL_DIR/sing-box" rule-set compile \
        /tmp/geoip-cn.json -o "$geoip_srs" 2>/dev/null || {
        red "✗ geoip-cn.srs 编译失败"; return 1
    }

    log "编译 geosite-cn.srs..."
    "$SINGBOX_INSTALL_DIR/sing-box" geosite export cn \
        -f "$SINGBOX_INSTALL_DIR/geosite.db" \
        -o /tmp/geosite-cn.json 2>/dev/null && \
    "$SINGBOX_INSTALL_DIR/sing-box" rule-set compile \
        /tmp/geosite-cn.json -o "$geosite_srs" 2>/dev/null || {
        red "✗ geosite-cn.srs 编译失败"; return 1
    }

    # 清理临时文件
    rm -f /tmp/geoip-cn.json /tmp/geosite-cn.json

    # 验证
    if [[ -f "$geoip_srs" && -f "$geosite_srs" ]]; then
        green "✓ geoip/geosite 规则集准备完成"
    else
        red "✗ 规则集文件缺失"; return 1
    fi
}
...
# -----------------------------------------------------------------------------
# IP优选: 重新注册 WARP 以更换出口IP
# 保留旧配置作为回退, 新注册不通则自动恢复
# 兼容自定义/全局两种模式
# -----------------------------------------------------------------------------
warp_ip_optimize() {
    local warp_key_file="$SINGBOX_INSTALL_DIR/.warp_wireguard.json"
    ...
    for i in $(seq 1 10); do
        ...
        local route_rules final_outbound=""
        if [[ "$cur_mode" == "global" ]]; then
            route_rules=$SINGBOX_GLOBAL_WARP_ROUTE_RULES
            final_outbound="warp-ep"
        else
            local warp_domains; warp_domains=$(get_warp_domain_list)
            [[ -z "$warp_domains" ]] && { green "所有域名直连可达，无需 WARP。"; return 0; }
            route_rules=$SINGBOX_DIRECT_ROUTE_RULES
            route_rules=${route_rules/__WARP_DOMAIN_SUFFIX_LIST__/$warp_domains}
        fi
        apply_warp_config_to_file "$wg_ep" "$route_rules" "$final_outbound" || continue
        systemctl restart ax-singbox.service 2>/dev/null
        local new_ip=""
        for w in 2 4 6; do
            sleep 2
    ...
    red "IP优选失败: 5 次注册均未更换出口IP。恢复原配置..."
    ...
    if [[ "$cur_mode" == "global" ]]; then
        route_rules=$SINGBOX_GLOBAL_WARP_ROUTE_RULES
        final_outbound="warp-ep"
        apply_warp_config_to_file "$wg_ep" "$route_rules" "$final_outbound"
    else
        local warp_domains; warp_domains=$(get_warp_domain_list)
...
warp_management_menu() {
    ...
    while true; do
        ...
        if $cfg_enabled; then
            ...
            if [[ "$warp_mode" == "global" ]]; then
                echo " 1) 禁用 WARP 分流"
                echo " 2) 切换至自定义 WARP 分流"
                echo " 3) IP优选 (更换出口IP)"
            else
                echo " 1) 禁用 WARP 分流"
                echo " 2) 编辑 WARP 分流域名"
                echo " 3) 切换至全局 WARP 分流"
                echo " 4) IP优选 (更换出口IP)"
            fi
        else
            yellow "当前状态: 未启用"
            echo "----------------------------------"
            echo " 1) 启用自定义 WARP 分流"
            echo " 2) 启用全局 WARP 分流"
            echo " 3) 编辑 WARP 分流域名"
        fi
        echo " 0) 返回"
        ...
        if $cfg_enabled; then
            if [[ "$warp_mode" == "global" ]]; then
                case $warp_choice in
                    1)
                        if disable_warp_in_config; then sync; systemctl restart ax-singbox.service 2>/dev/null; green "Sing-box 已重启。"; else yellow "禁用失败。"; fi
                        read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
                    2)
                        yellow "正在切换至自定义 WARP 分流..."
                        if enable_warp_in_config; then
                            green "✓ 已切换至自定义 WARP 分流"
                        else
                            yellow "切换失败。"
                        fi
                        read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
                    3)
                        if warp_ip_optimize; then green "出口IP已更换"; else yellow "所有尝试均失败，已恢复原配置。"; fi
                        read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
                    0) break;;
                    *) red "无效选择!"; sleep 1;;
                esac
                ...
                case $warp_choice in
                    ...
                    2) edit_warp_domains;;
                    3)
                        yellow "正在切换至全局 WARP 分流..."
                        if enable_global_warp_in_config; then
                            green "✓ 已切换至全局 WARP 分流"
                        else
                            yellow "切换失败。"
                        fi
                        read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
            ...
            case $warp_choice in
                1)
                    if enable_warp_in_config; then sync; green "自定义 WARP 分流已启用！"; else yellow "启用失败。"; fi
                    read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
                2)
                    if enable_global_warp_in_config; then sync; green "全局 WARP 分流已启用！"; else yellow "启用失败。"; fi
                    read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
                3) edit_warp_domains;;
                0) break;;
                *) red "无效选择!"; sleep 1;;
...
generate_singbox_reality_link() {
    ...
    local link="vless://${uuid}@${ip}:${port}?type=tcp&security=reality&sni=${sni}&pbk=${pbk}&flow=${flow}&sid=${sid}&fp=chrome#$(get_instance_display_name "xray_reality" "$id")"
    echo "$link"
}
...
generate_singbox_hy2_link() {
    ...
    local link="hysteria2://${password}@${ip}:${port}?sni=${sni}&insecure=1#$(get_instance_display_name "hysteria2" "$id")"
    echo "$link"
}
...
view_chain_client_config_3() {
    ...
    local sub_link="ss://${ss_user_info}@${ss_host}:${ss_port}#$(get_instance_display_name "ss_3_chain_chain" "$id_num")"
    green "$sub_link"; echo
...
view_chain_client_config() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        sub_link="hysteria2://${hy2_password}@${client_udp2raw_host}:${client_udp2raw_port}?sni=${sni}&insecure=${insecure_flag}#$(get_instance_display_name "hy2_chain" "$id_num")"
...
" 2>/dev/null && systemctl restart ax-singbox.service 2>/dev/null
        ...
        local mode=$(get_warp_mode)
        if [[ "$mode" == "global" ]]; then
            green "WARP 分流: 已启用 (全局)"
        else
            green "WARP 分流: 已启用 (自定义)"
        fi
