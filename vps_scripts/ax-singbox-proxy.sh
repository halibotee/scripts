...
safe_add_singbox_inbound() {
    ...
    if ! command -v jq &>/dev/null; then
        red "错误: jq 未安装，无法修改 singbox 配置。" >&2
        return 1
    fi
    # 预校验 inbound JSON 合法性
    if ! echo "$inbound_json" | jq empty 2>/dev/null; then
...
remove_singbox_inbound() {
    ...
    local tmpfile
    tmpfile=$(umask 077 && mktemp) || { red "无法创建临时文件" >&2; return 1; }
    if ! jq --arg tag "$tag" 'del(.inbounds[] | select(.tag == $tag))' "$config_file" > "$tmpfile" 2>/dev/null; then
        rm -f "$tmpfile"
        red "错误: jq 移除失败" >&2
        return 1
    fi
