...
# 参数: $1=endpoint JSON, $2=route_rules JSON
# 写入成功返回 0, 失败返回 1
# -----------------------------------------------------------------------------
...
warp_management_menu() {
 ...
 2) edit_warp_domains;;
                3)
                    if warp_ip_optimize; then
                        green "出口IP已更换"
                    else
                        yellow "所有尝试均失败，已恢复原配置。"
                    fi
                    read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
 0) break;;
                *) red "无效选择!"; sleep 1;;
            ...
            case $warp_choice in
                1) if enable_warp_in_config; then sync; green "Sing-box 已重启。"; else yellow "启用失败。"; fi; read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
 2) edit_warp_domains;;
 0) break;;
                *) red "无效选择!"; sleep 1;;
            esac
            ...
            case $warp_choice in
                1)
                    if enable_warp_in_config; then sync; green "WARP 已启用！"; else yellow "启用失败。"; fi
                    read -p $'\n按任意键返回主菜单...' -n1 -s; break;;
 2) edit_warp_domains;;
 0) break;;
                *) red "无效选择!"; sleep 1;;
