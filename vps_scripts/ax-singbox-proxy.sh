...
cfg['route']['rules'] = rules
    if '$final_outbound':
        ...
        cfg['route']['geoip_path'] = '$SINGBOX_INSTALL_DIR/geoip.db'
        cfg['route']['geosite_path'] = '$SINGBOX_INSTALL_DIR/geosite.db'
...
if 'final' in cfg.get('route', {}):
    del cfg['route']['final']
for k in ['geoip_path', 'geosite_path']:
    cfg['route'].pop(k, None)
...
download_geoip_dbs() {
    local need_dl=false
    if [[ ! -f "$SINGBOX_INSTALL_DIR/geoip.db" ]]; then need_dl=true; fi
    if [[ ! -f "$SINGBOX_INSTALL_DIR/geosite.db" ]]; then need_dl=true; fi
    $need_dl || { green "✓ geoip/geosite 数据库已存在"; return 0; }
