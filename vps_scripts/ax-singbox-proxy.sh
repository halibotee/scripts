...
warp_ruleset_count() {
    local list_file="$SINGBOX_INSTALL_DIR/.warp_ruleset_list"
    if [[ -f "$list_file" ]]; then
        grep -cE '^[a-zA-Z0-9_-]' "$list_file" 2>/dev/null || echo 0
    else
        echo 0
    fi
}
...
edit_warp_rulesets() {
    ...
    if [[ ! -f "$list_file" ]]; then
        cat > "$list_file" <<'EOF'
# 每行一个规则集名称, # 开头为注释行(不启用)
# 可用规则集参考: https://github.com/SagerNet/sing-geosite/tree/rule-set
geosite-openai
