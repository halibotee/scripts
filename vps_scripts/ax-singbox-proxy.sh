...
register_warp_wireguard() {
    ...
    local cid=$(echo "$api_result" | jq -r '.client_id // empty' 2>/dev/null)
    ...
    # 解码 client_id -> reserved (3 字节)
    local reserved="[0, 0, 0]"
...
apply_warp_wireguard_config() {
    ...
    if [[ -z "$reserved" || "$reserved" == "null" ]]; then
        yellow "警告: WARP reserved 缺失, 使用 [0,0,0]。" >&2
        reserved="[0, 0, 0]"
    fi
