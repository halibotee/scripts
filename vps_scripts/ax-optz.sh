#!/bin/bash
# VPS优化脚本，包含网络参数调优、BBR加速、ZRAM内存压缩、Fail2ban安全防护、SELinux禁用等功能 
...
fn_setup_fail2ban() {
    ...
    # 询问用户是否安装 Fail2ban
    local install_fail2ban="n"
    if [ $FORCE_YES -eq 1 ]; then
        install_fail2ban="n"
        echo "非交互模式: 默认不安装 Fail2ban (使用 -y 标志)。"
        fn_log "信息" "非交互模式: 默认不安装 Fail2ban。"
        return 3
    else
        read -rp "是否安装 Fail2ban 安全防护? (y/N): " install_fail2ban || install_fail2ban="n"
        # 如果用户直接按回车(空字符串)或输入 n/N，则不安装
        if [ -z "$install_fail2ban" ] || [ "$install_fail2ban" = "n" ] || [ "$install_fail2ban" = "N" ]; then
            echo "跳过 Fail2ban 安装。"
            fn_log "信息" "用户选择跳过 Fail2ban 安装。"
            return 3
        fi
    fi
...
fn_setup_aggressive_network() {
    ...
    if [ $FORCE_YES -eq 1 ]; then
        ...
        echo "  说明: VPS内存将自动获取 (当前: ${vps_mem_default}MB, 分层: ${MEMORY_TIER})"
        echo ""
...
# 延迟: ${NETWORK_LATENCY}ms, 本地带宽: ${NETWORK_LOCAL_BW}Mbps, VPS带宽: ${NETWORK_VPS_BW}Mbps, 内存: ${NETWORK_VPS_MEM}MB, 内存分层: ${MEMORY_TIER}
...
fn_enable_bbr_fq() {
    ...
    # 尝试加载 BBR 内核模块 (v1, v2, v3)
    local bbr_loaded=""
    if modprobe tcp_bbr 2>/dev/null; then
        bbr_loaded="tcp_bbr"
        fn_log "信息" "已加载 BBR v1 模块"
    fi
    if modprobe tcp_bbr2 2>/dev/null; then
        bbr_loaded="tcp_bbr2"
        fn_log "信息" "已加载 BBR v2 模块"
    elif [ -z "$bbr_loaded" ] && modprobe tcp_bbr3 2>/dev/null; then
        bbr_loaded="tcp_bbr3"
        fn_log "信息" "已加载 BBR v3 模块"
    fi
    if [ -z "$bbr_loaded" ]; then
        fn_log "警告" "无法加载 BBR 模块 (内核可能不支持)"
    fi
...
fn_tune_network_interface() {
    ...
    if command -v ethtool > /dev/null 2>&1; then
        # 仅禁用 GRO (降低轻微延迟)，保留 GSO/TSO (提高代理吞吐量)
        ethtool -K "$default_iface" gro off > /dev/null 2>&1 || true
        fn_log "信息" "已禁用 GRO (ethtool)，保留 GSO/TSO 以保证吞吐。"
    fi
...
fn_setup_systemd_limits() {
    fn_log "信息" "配置 systemd 默认资源限制..."

    fn_detect_memory_tier

    local nofile_default nproc_default
    case "$MEMORY_TIER" in
        tiny)
            nofile_default=524288
            nproc_default=32768
            ;;
        small)
            nofile_default=524288
    ...
    if ! grep -q "vps_optimizert" "$sysconf" 2>/dev/null; then
        cp -an "$sysconf" "${sysconf}.bak" 2>/dev/null || true
        cat >> "$sysconf" <<EOF

# vps_optimizert
DefaultLimitNOFILE=${nofile_default}
DefaultLimitNPROC=${nproc_default}
EOF
        fn_log_action "MODIFY_FILE" "$sysconf"
        changes_made=1
    else
        fn_log "信息" "systemd 默认限制已配置，跳过。"
        return 2
    fi

    if [ "$changes_made" -eq 1 ]; then
        systemctl daemon-reexec > /dev/null 2>&1 || fn_log "警告" "systemd daemon-reexec 失败"
        fn_log "成功" "systemd 默认资源限制已配置 (nofile=${nofile_default}, nproc=${nproc_default})。"
    fi
    return 0
}

_fn_ensure_boot_script() {
    local script_file="/etc/vps_optimizert/boot.sh"
    local svc_file="/etc/systemd/system/vps-optimizert-boot.service"

    if [ -f "$svc_file" ]; then
        return 0
    fi

    mkdir -p /etc/vps_optimizert

    cat > "$script_file" <<'SHEOF'
#!/bin/sh
# VPS Optimizert - Boot-time tweaks
...
for sched_file in /sys/block/*/queue/scheduler; do
    dev=$(echo "$sched_file" | cut -d/ -f4)
    rotational=$(cat "/sys/block/$dev/queue/rotational" 2>/dev/null || echo 1)
    if [ "$rotational" = "0" ]; then
        echo none > "$sched_file" 2>/dev/null || echo mq-deadline > "$sched_file" 2>/dev/null || true
...
SHEOF
    ...
    cat > "$svc_file" <<'UNIT'
[Unit]
Description=VPS Optimizert Boot Tweaks
After=multi-user.target

[Service]
Type=oneshot
ExecStart=/etc/vps_optimizert/boot.sh
RemainAfterExit=yes
...
UNIT
    ...
    systemctl enable vps-optimizert-boot.service >/dev/null 2>&1 || true
    fn_log_action "CREATE_FILE" "$svc_file"
    fn_log_action "CREATE_FILE" "$script_file"
    fn_log "成功" "启动持久化脚本已创建。"
}

fn_setup_noatime() {
    fn_log "信息" "配置 noatime 挂载参数..."

    local fstab="/etc/fstab"
    local changes_made=0

    while IFS=$' \t' read -r fs mnt type opts rest; do
        [ -z "$fs" ] && continue
        [[ "$fs" == '#'* ]] && continue
        [ "$mnt" != '/' ] && [ "$mnt" != '/home' ] && [ "$mnt" != '/var' ] && continue

        if [[ "$opts" != *noatime* ]]; then
            local new_opts="noatime,${opts}"
            sed -i "s|^${fs}[[:blank:]]\\+${mnt}[[:blank:]]\\+${type}[[:blank:]]\\+${opts}|${fs}\t${mnt}\t${type}\t${new_opts}|" "$fstab"
            fn_log_action "MODIFY_FILE" "$fstab"
            mount -o remount "$mnt" 2>/dev/null || true
            fn_log "信息" "已设置 $mnt noatime"
            changes_made=1
        fi
    done < <(grep -v '^#' "$fstab" | grep -v '^$')

    if [ "$changes_made" -eq 0 ]; then
        fn_log "信息" "noatime 已配置，跳过。"
        return 2
    fi

    fn_log "成功" "noatime 挂载参数已配置。"
    return 0
}
...
fn_setup_cpu_governor() {
    fn_log "信息" "设置 CPU 调度器为 performance..."

    local gov="/sys/devices/system/cpu/cpu0/cpufreq/scaling_governor"
    if [ ! -f "$gov" ]; then
        fn_log "信息" "CPU 频率调节不可用，跳过。"
        return 2
    fi
    ...
    if [ "$current" = "performance" ]; then
        fn_log "信息" "CPU governor 已是 performance，跳过。"
        _fn_ensure_boot_script
        return 2
    fi
    ...
    _fn_ensure_boot_script
    fn_log "成功" "CPU governor 已设置为 performance。"
    return 0
}

fn_disable_thp() {
    fn_log "信息" "禁用透明大页 (THP)..."

    local thp_path="/sys/kernel/mm/transparent_hugepage/enabled"
    if [ ! -f "$thp_path" ]; then
        fn_log "信息" "THP 不可用，跳过。"
        return 2
    fi
    ...
    if [ -f "$grub_file" ]; then
        if ! grep -q "transparent_hugepage=never" "$grub_file"; then
            sed -i 's/GRUB_CMDLINE_LINUX="\(.*\)"/GRUB_CMDLINE_LINUX="\1 transparent_hugepage=never"/' "$grub_file"
            fn_log_action "MODIFY_FILE" "$grub_file"
...
fn_setup_haveged() {
    fn_log "信息" "配置 haveged (熵服务)..."

    if systemctl is-active haveged >/dev/null 2>&1; then
        fn_log "信息" "haveged 已激活，跳过。"
        return 2
    fi

    if ! "${PKG_CMD_CHECK[@]}" haveged >/dev/null 2>&1; then
        fn_log "信息" "正在安装 haveged..."
        fn_wait_for_pkg_lock || { fn_log "错误" "包管理器锁等待失败"; return 1; }
        "${PKG_CMD_INSTALL[@]}" haveged >>"$LOG_FILE" 2>&1 || {
            fn_log "警告" "haveged 安装失败";
            return 1;
        }
        fn_log_action "INSTALL_PKG" "haveged"
    fi

    systemctl enable --now haveged >/dev/null 2>&1 || true

    if systemctl is-active haveged >/dev/null 2>&1; then
        fn_log "成功" "haveged 已激活。"
        return 0
    else
        fn_log "错误" "haveged 启动失败。"
        return 1
    fi
}

fn_disable_idle_services() {
    fn_log "信息" "禁用不必要的系统服务和定时器..."
    local changes_made=0
...
fn_optimize_auto() {
    ...
    echo "[任务 13]: 配置 noatime 挂载参数..."
    result=0
    fn_setup_noatime || result=$?
    if [ $result -eq 0 ]; then
        echo "[完成] noatime 已配置。"
    elif [ $result -eq 2 ]; then
        echo "[跳过] noatime 已配置。"
    ...
    echo "[任务 14]: 配置 earlyoom (OOM 保护)..."
    result=0
    fn_setup_earlyoom || result=$?
    if [ $result -eq 0 ]; then
        echo "[完成] earlyoom 已激活。"
    elif [ $result -eq 2 ]; then
        echo "[跳过] earlyoom 已激活。"
    else
        echo "[警告] earlyoom 配置失败。"
    fi
    echo ""

    echo "[任务 15]: 设置 CPU 调度器为 performance..."
    result=0
    fn_setup_cpu_governor || result=$?
    if [ $result -eq 0 ]; then
        echo "[完成] CPU governor 已设置为 performance。"
    elif [ $result -eq 2 ]; then
        echo "[跳过] 已优化或不可用。"
    fi
    echo ""

    echo "[任务 16]: 禁用透明大页 (THP)..."
    result=0
    fn_disable_thp || result=$?
    if [ $result -eq 0 ]; then
        echo "[完成] THP 已禁用。"
    elif [ $result -eq 2 ]; then
        echo "[跳过] THP 已禁用或不可用。"
    ...
    echo "[任务 17]: 配置 haveged (熵服务)..."
    result=0
    fn_setup_haveged || result=$?
    if [ $result -eq 0 ]; then
        echo "[完成] haveged 已激活。"
    elif [ $result -eq 2 ]; then
        echo "[跳过] haveged 已激活。"
    else
        echo "[警告] haveged 配置失败。"
    fi
    echo ""

    echo "[任务 18]: 禁用不必要的系统服务和定时器..."
    result=0
    fn_disable_idle_services || result=$?
    if [ $result -eq 0 ]; then
        echo "[完成] 闲置服务优化完成。"
    elif [ $result -eq 2 ]; then
        echo "[跳过] 无需优化的闲置服务。"
    ...
    echo "[任务 19]: 配置 systemd 默认资源限制..."
    result=0
    fn_setup_systemd_limits || result=$?
    if [ $result -eq 0 ]; then
        echo "[完成] systemd 默认资源限制已配置。"
    elif [ $result -eq 2 ]; then
        echo "[跳过] systemd 限制已配置。"
    fi
    echo ""
...
EOF
    ...
    local files_to_clean=(
        ...
        "/etc/vps_optimizert/boot.sh"
        "/etc/systemd/system/vps-optimizert-boot.service"
    )
