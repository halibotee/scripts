...
# 2. 脚本配置
# =============================================================================
SCRIPT_VERSION="1.0.0 from halibotee"
...
renew_certificate() {
    ...
    log "正在续订证书 $domain ..."
    "$ACME_SH_INSTALL_DIR" --renew -d "$domain" --ecc --force
...
# =============================================================================
uninstall_acme() {
    read -p "确认要卸载 acme.sh 客户端吗？(将删除所有证书) [y/N]: " confirm
    if [[ "$confirm" != "y" && "$confirm" != "Y" ]]; then
        yellow "操作已取消。"
        return 0
    fi
