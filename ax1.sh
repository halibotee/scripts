...
# --- [Refactored] Generic GitHub Installer ---
install_from_github() {
    local repo=$1 pattern=$2 dir=$3 bin_name=$4 format=$5
    
    log "正在检查 $repo 最新版本..."
    local tag=$(get_latest_github_tag "$repo")
    if [[ -z "$tag" || "$tag" == "null" ]]; then red "获取 $repo 版本失败"; return 1; fi
    
    log "发现最新版本: $tag"
    mkdir -p "$dir"
    
    local api_url="$GITHUB_API_URL/$repo/releases/latest"
    local download_url=$(curl -s "$api_url" | jq -r --arg pat "$pattern" '.assets[] | select(.name | test($pat)) | .browser_download_url' | head -n1)
    
    if [[ -z "$download_url" ]]; then red "未找到匹配 ($pattern) 的资源"; return 1; fi
    
    local tmp_file="/tmp/ax_download_$(date +%s)"
    download_with_retry "$download_url" "$tmp_file" || return 1
    
    log "安装中..."
    if [[ "$format" == "zip" ]]; then
        unzip -o "$tmp_file" -d "$dir" 2>/dev/null
    elif [[ "$format" == "tar" ]]; then
        tar -xzf "$tmp_file" -C "$dir" 2>/dev/null
    else
        mv "$tmp_file" "$dir/$bin_name"
    fi
    
    if [[ "$bin_name" != "*" ]]; then
        if [[ ! -f "$dir/$bin_name" ]]; then
            find "$dir" -type f -name "*$bin_name*" -exec mv {} "$dir/$bin_name" \; 2>/dev/null
        fi
...
check_for_updates() {
    log "正在执行全量更新..."
    install_from_github "$KCPTUN_REPO" "linux-amd64" "$KCPTUN_INSTALL_DIR" "kcptun_server" "tar"
    install_from_github "$UDP2RAW_REPO" "amd64" "$UDP2RAW_INSTALL_DIR" "udp2raw" "tar"
    install_from_github "$HY2_REPO" "linux-amd64" "$HY2_INSTALL_DIR" "hysteria" "bin"
    install_from_github "$XRAY_REPO" "linux-64.zip" "$XRAY_INSTALL_DIR" "xray" "zip"
...
# 下载 KCPTUN 和 UDP2RAW
download_kcp_udp_binaries(){ install_from_github "$KCPTUN_REPO" "linux-amd64" "$KCPTUN_INSTALL_DIR" "kcptun_server" "tar"; install_from_github "$UDP2RAW_REPO" "amd64" "$UDP2RAW_INSTALL_DIR" "udp2raw" "tar"; }

# 下载 Hysteria2
download_hysteria2_binary(){ install_from_github "$HY2_REPO" "linux-amd64" "$HY2_INSTALL_DIR" "hysteria" "bin"; }

# 下载 Xray-core
download_xray_binary(){ install_from_github "$XRAY_REPO" "linux-64.zip" "$XRAY_INSTALL_DIR" "xray" "zip"; cp -n "$XRAY_INSTALL_DIR/geoip.dat" "$HY2_INSTALL_DIR/" 2>/dev/null; }
