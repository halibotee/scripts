...
download_kcp_udp_binaries(){
    ...
    tar -xzf /tmp/kcptun.tar.gz -C "$KCP_INSTALL_DIR" server_linux_amd64 && mv "$KCP_INSTALL_DIR/server_linux_amd64" "$KCP_INSTALL_DIR/kcptun_server" || { red "KCPTUN 解压或移动失败。"; return 1; }
    ...
    chmod +x "$KCP_INSTALL_DIR/kcptun_server" "$UDP2RAW_INSTALL_DIR/udp2raw"
}
    ...
    case "$type" in
        ...
        "hysteria2"|"udp2raw"|"kcptun"|"xray_reality"|"xray_mkcp"|"xray_ss")
            ...
            case "$type" in
                ...
                udp2raw) conf_file="$UDP2RAW_INSTALL_DIR/udp2raw_${id}.conf"; service_prefix="udp2raw"; title="UDP2RAW";;
                kcptun) conf_file="$KCP_INSTALL_DIR/kcptun_${id}.json"; service_prefix="kcptun"; title="KCPTUN";;
...
create_new_instance() {
    ...
    case "$type" in
        hysteria2) 
            ...
            SERVICE_PREFIX="hy2"
            SYSTEMD_SERVICE_NAME="hysteria2"
            FILE_EXT="yaml"
            ...
            # 添加更多Hysteria2配置选项
            cyan "--- Hysteria2 高级配置选项 ---"
            read -p "是否忽略客户端带宽限制 (ignoreClientBandwidth)? (默认 true) [True/false]: " ignore_client_bandwidth
            ...
            read -p "是否启用拥塞控制 (congestion)? (默认 true) [True/false]: " congestion_enabled
            congestion_enabled=${congestion_enabled:-true}
            if [[ "$congestion_enabled" != "false" ]]; then
                congestion_enabled="true"
            fi

            if [[ "$congestion_enabled" == "true" ]]; then
                read -p "拥塞控制类型 (bbr/cubic) (默认 bbr): " congestion_type
                congestion_type=${congestion_type:-bbr}
                replacements["__CONGESTION_TYPE__"]="$congestion_type"
                
                read -p "下行带宽限制 (MB/s) (默认 0 不限制): " congestion_down
                congestion_down=${congestion_down:-0}
                replacements["__CONGESTION_DOWN__"]="$congestion_down"
                
                read -p "上行带宽限制 (MB/s) (默认 0 不限制): " congestion_up
                congestion_up=${congestion_up:-0}
                replacements["__CONGESTION_UP__"]="$congestion_up"
            else
                replacements["__CONGESTION_TYPE__"]="bbr"
                replacements["__CONGESTION_DOWN__"]="0"
                replacements["__CONGESTION_UP__"]="0"
            fi
            ...
            ;;
        udp2raw) TITLE="UDP2RAW"; INSTALL_DIR="$UDP2RAW_INSTALL_DIR"; SERVICE_PREFIX="udp2raw"; SYSTEMD_SERVICE_NAME="udp2raw"; FILE_EXT="conf"; CONFIG_TEMPLATE="$UDP2RAW_CONFIG_TEMPLATE" 
            log "启动一个新的 ${TITLE} 实例..."; next_id=$(find_next_available_id "$type")
            green "新实例将被创建为: ${SYSTEMD_SERVICE_NAME}@${next_id}.service"; cyan "--- 配置新实例 (ID: ${next_id}) ---"
            ...
            # 添加UDP2RAW高级配置选项
            cyan "--- UDP2RAW 高级配置选项 ---"
            read -p "Raw模式 (faketcp/udp/icm) (默认 faketcp): " raw_mode
            raw_mode=${raw_mode:-faketcp}
            replacements["__RAW_MODE__"]="$raw_mode"
            
            read -p "加密模式 (aes128cbc/xor) (默认 aes128cbc): " cipher_mode
            cipher_mode=${cipher_mode:-aes128cbc}
            replacements["__CIPHER_MODE__"]="$cipher_mode"
            
            read -p "认证模式 (hmac_sha1/simple) (默认 hmac_sha1): " auth_mode
        ...
        kcptun) TITLE="KCPTUN"; INSTALL_DIR="$KCP_INSTALL_DIR"; SERVICE_PREFIX="kcptun"; SYSTEMD_SERVICE_NAME="kcptun"; FILE_EXT="json"; CONFIG_TEMPLATE="$KCPTUN_CONFIG_JSON_TEMPLATE" 
            log "启动一个新的 ${TITLE} 实例..."; next_id=$(find_next_available_id "$type")
            green "新实例将被创建为: ${SYSTEMD_SERVICE_NAME}@${next_id}.service"; cyan "--- 配置新实例 (ID: ${next_id}) ---"
            read -p "请输入监听地址 (默认: $DEFAULT_LISTEN_ADDR): " listen_addr_input; listen_addr=${listen_addr_input:-$DEFAULT_LISTEN_ADDR}
            read -p "请输入监听端口 (留空则随机生成): " listen_port; if [[ -z "$listen_port" ]]; then listen_port=$(find_available_port); green "已为您随机选择端口: $listen_port"; fi
            ...
            # 添加KCPTUN高级配置选项
            cyan "--- KCPTUN 高级配置选项 ---"
            read -p "加速模式 (默认: $KCPTUN_DEFAULT_MODE): " mode
            mode=${mode:-$KCPTUN_DEFAULT_MODE}
            replacements["__MODE__"]="$mode"
            
            read -p "加密方式 (aes-128/aes-192/aes-256) (默认 aes-128): " crypt
            crypt=${crypt:-aes-128}
            replacements["__CRYPT__"]="$crypt"
            
            read -p "是否启用TCP (默认 false) [True/false]: " tcp_enabled
            ...
            read -p "是否启用数据压缩 (默认 true) [True/false]: " compression_enabled
...
create_new_xray_instance() {
    ...
    if [[ "$type" == "xray_reality" ]]; then
        ...
        # 添加mKCP高级配置选项
        cyan "--- mKCP 高级配置选项 ---"
        read -p "是否启用拥塞控制 (默认 true) [True/false]: " mkcp_congestion
...
start_new_chain_instance_3() {
    ...
    # KCPTUN配置选项
    read -p "KCPTUN加速模式 (默认: $KCPTUN_DEFAULT_MODE): " kcp_mode
    kcp_mode=${kcp_mode:-$KCPTUN_DEFAULT_MODE}
    kcp_config=${kcp_config/__MODE__/$kcp_mode}
    
    read -p "KCPTUN加密方式 (aes-128/aes-192/aes-256) (默认 aes-128): " kcp_crypt
    kcp_crypt=${kcp_crypt:-aes-128}
    kcp_config=${kcp_config/__CRYPT__/$kcp_crypt}
    
    read -p "KCPTUN是否启用TCP (默认 false) [True/false]: " kcp_tcp
    kcp_tcp=${kcp_tcp:-false}
    if [[ "$kcp_tcp" != "true" ]]; then
        kcp_tcp="false"
    fi
    kcp_config=${kcp_config/__TCP__/$kcp_tcp}
    
    read -p "KCPTUN是否启用数据压缩 (默认 true) [True/false]: " kcp_nocomp
    kcp_nocomp=${kcp_nocomp:-true}
    if [[ "$kcp_nocomp" != "false" ]]; then
        kcp_nocomp="false"  # 注意：nocomp为true表示不压缩，所以这里要取反
    else
        kcp_nocomp="true"
    fi
    ...
    # UDP2RAW配置选项
    read -p "UDP2RAW Raw模式 (faketcp/udp/icm) (默认 faketcp): " udp_raw_mode
    udp_raw_mode=${udp_raw_mode:-faketcp}
    udp_config="${udp_config//--raw-mode faketcp/--raw-mode $udp_raw_mode}"
    
    read -p "UDP2RAW 加密模式 (aes128cbc/xor) (默认 aes128cbc): " udp_cipher_mode
    udp_cipher_mode=${udp_cipher_mode:-aes128cbc}
    udp_config="${udp_config//--cipher-mode aes128cbc/--cipher-mode $udp_cipher_mode}"
    
    read -p "UDP2RAW 认证模式 (hmac_sha1/simple) (默认 hmac_sha1): " udp_auth_mode
    udp_auth_mode=${udp_auth_mode:-hmac_sha1}
    udp_config="${udp_config//--auth-mode hmac_sha1/--auth-mode $udp_auth_mode}"
...
start_new_chain_instance() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        # 添加Hysteria2串联实例的高级配置选项
        cyan "--- Hysteria2 串联实例高级配置 ---"
        read -p "是否忽略客户端带宽限制 (ignoreClientBandwidth)? (默认 true) [True/false]: " ignore_client_bandwidth
        ignore_client_bandwidth=${ignore_client_bandwidth:-true}
        if [[ "$ignore_client_bandwidth" != "false" ]]; then
            ignore_client_bandwidth="true"
        ...
        # 添加VLESS mKCP串联实例的高级配置选项
        cyan "--- VLESS mKCP 串联实例高级配置 ---"
        read -p "是否启用拥塞控制 (默认 true) [True/false]: " mkcp_congestion
        mkcp_congestion=${mkcp_congestion:-true}
        if [[ "$mkcp_congestion" != "false" ]]; then
            mkcp_congestion="true"
    ...
    # 添加UDP2RAW串联实例的高级配置选项
    cyan "--- UDP2RAW 串联实例高级配置 ---"
    read -p "Raw模式 (faketcp/udp/icm) (默认 faketcp): " raw_mode
    raw_mode=${raw_mode:-faketcp}
    temp_udp_config="${temp_udp_config//--raw-mode faketcp/--raw-mode $raw_mode}"
    
    read -p "加密模式 (aes128cbc/xor) (默认 aes128cbc): " cipher_mode
    cipher_mode=${cipher_mode:-aes128cbc}
    temp_udp_config="${temp_udp_config//--cipher-mode aes128cbc/--cipher-mode $cipher_mode}"
    
    read -p "认证模式 (hmac_sha1/simple) (默认 hmac_sha1): " auth_mode
    auth_mode=${auth_mode:-hmac_sha1}
    temp_udp_config="${temp_udp_config//--auth-mode hmac_sha1/--auth-mode $auth_mode}"
...
# 重启所有服务
restart_all_services(){ log "正在重启所有正在运行的实例..."; systemctl restart ax-kcptun@*.service ax-udp2raw@*.service ${XRAY_SERVICE_NAME}@*.service 2>/dev/null; green "操作完成！"; sleep 2; }
...
uninstall_all() {
    ...
    systemctl stop "ax-kcptun@*.service" "ax-udp2raw@*.service" "${XRAY_SERVICE_NAME}@*.service" "hysteria-server@*.service" 2>/dev/null
    systemctl disable "ax-kcptun@*.service" "ax-udp2raw@*.service" "${XRAY_SERVICE_NAME}@*.service" "hysteria-server@*.service" 2>/dev/null
    green "服务已停止并禁用。"
    ...
    while true; do
        ...
        if [[ -n "${QUICK_MANAGE_MAP_ID[$choice]}" ]]; then
            ...
            case "$type" in
                ...
                "hysteria2") manage_instance_menu "hysteria2" "$real_id" "${XRAY_SERVICE_NAME}@${real_id}" "$HY2_INSTALL_DIR/hy2_${real_id}.yaml" ;;
