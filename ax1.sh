...
# --- [Refactored] 统一安装器 (Ported from AX.sh v2.4.1) ---
install_bin() {
    local repo=$1 pattern=$2 dest_dir=$3 bin_name=$4 type=$5 # type: zip/tar/bin
    
    # 检查是否已存在且可执行
    if [[ -f "$dest_dir/$bin_name" && -x "$dest_dir/$bin_name" ]]; then
        # 简单的存在性检查，如果需要强制更新，调用者应先删除文件
        return 0
    fi
    
    log "正在安装 $bin_name (源: $repo)..."
    
    # 确保目录存在
    mkdir -p "$dest_dir"
    
    # 获取下载链接 (依赖 jq)
    if ! command -v jq &>/dev/null; then red "错误: 缺少 jq 组件，无法解析 API。"; return 1; fi
    
    local api_url="$GITHUB_API_URL/$repo/releases/latest"
    local download_url=$(curl -s "$api_url" | jq -r --arg p "$pattern" '.assets[]|select(.name|test($p))|.browser_download_url' | head -1)
    
    if [[ -z "$download_url" || "$download_url" == "null" ]]; then 
        red "未找到 $repo 的发布资源 (匹配规则: $pattern)"; return 1
    fi
    
    local tmp="/tmp/ax_dl_$(date +%s)"
    # 使用原有 retry 逻辑或直接 curl
    download_with_retry "$download_url" "$tmp" || return 1
    
    case "$type" in
        zip) unzip -o "$tmp" -d "$dest_dir" >/dev/null 2>&1 ;;
        tar) tar -xzf "$tmp" -C "$dest_dir" >/dev/null 2>&1 ;;
        bin) mv "$tmp" "$dest_dir/$bin_name" ;;
    esac
    rm -f "$tmp"

    # --- 特殊后续处理 ---
    # 1. Xray: 复制 Geo 文件到 Hy2 目录 (适配 ax1.sh 逻辑)
    if [[ "$bin_name" == "xray" ]]; then
        cp -n "$dest_dir/geoip.dat" "$HY2_INSTALL_DIR/" 2>/dev/null
        cp -n "$dest_dir/geosite.dat" "$HY2_INSTALL_DIR/" 2>/dev/null
    fi
    
    # 2. UDP2RAW: 提取并重命名 (处理 udp2raw_binaries.tar.gz 结构)
    if [[ "$bin_name" == "udp2raw" ]]; then
        # ax1.sh 需要 udp2raw 二进制在根目录
        if [[ -f "$dest_dir/udp2raw_amd64" ]]; then
            mv "$dest_dir/udp2raw_amd64" "$dest_dir/udp2raw"
        fi
        # 清理其他架构的垃圾文件
        rm -f "$dest_dir"/udp2raw_x86 "$dest_dir"/udp2raw_arm* "$dest_dir"/udp2raw_mips*
    fi
    
    # 3. KCPTUN: 确保文件名正确
    if [[ "$bin_name" == "kcptun_server" ]]; then
        # kcptun tar 包解压后通常是 server_linux_amd64
        if [[ -f "$dest_dir/server_linux_amd64" ]]; then
             mv "$dest_dir/server_linux_amd64" "$dest_dir/kcptun_server"
        fi
        rm -f "$dest_dir"/client_linux_amd64
    fi

    chmod +x "$dest_dir/$bin_name"
    green "$bin_name 安装/更新 完成。"
}

# 统一更新/安装入口
run_install_all() {
    # KCPTUN
    install_bin "$KCPTUN_REPO" "linux-amd64" "$KCPTUN_INSTALL_DIR" "kcptun_server" "tar"
    # UDP2RAW (修复规则: 使用 tar.gz 匹配)
    install_bin "$UDP2RAW_REPO" "tar.gz" "$UDP2RAW_INSTALL_DIR" "udp2raw" "tar"
    # Hysteria2
    install_bin "$HY2_REPO" "linux-amd64" "$HY2_INSTALL_DIR" "hysteria" "bin"
    # Xray
    install_bin "$XRAY_REPO" "linux-64.zip" "$XRAY_INSTALL_DIR" "xray" "zip"
}
...
# [Restored] 安装 VPS 一键优化脚本
install_sys_opt() {
    log "即将下载并运行 VPS 优化脚本..."
...
initial_check_and_install() {
    ...
    [[ ! -f "$KCPTUN_INSTALL_DIR/kcptun_server" ]] && bin_missing=true
    [[ ! -f "$UDP2RAW_INSTALL_DIR/udp2raw" ]] && bin_missing=true
    [[ ! -f "$HY2_INSTALL_DIR/hysteria" ]] && bin_missing=true
    [[ ! -f "$XRAY_INSTALL_DIR/xray" ]] && bin_missing=true
    
    if [[ "$bin_missing" == "true" ]]; then
        yellow "检测到核心组件缺失，开始安装..."
        run_install_all
    fi
