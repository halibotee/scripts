...
start_new_chain_instance() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        local mtu=$(read_input "请输入 MTU (最大传输单元) [建议 1350]" "1350"); replacements["__MTU__"]="$mtu"
        main_config=${main_config/__MTU__/$mtu}
        
        local tti=$(read_input "请输入 TTI (传输时间间隔 ms) [建议 20-50]" "20"); replacements["__TTI__"]="$tti"
        main_config=${main_config/__TTI__/$tti}
        
        local uplink=$(read_input "请输入 uplinkCapacity (上行容量 MB/s)" "5"); replacements["__UPLINK__"]="$uplink"
        main_config=${main_config/__UPLINK__/$uplink}
        
        local downlink=$(read_input "请输入 downlinkCapacity (下行容量 MB/s)" "20"); replacements["__DOWNLINK__"]="$downlink"
        main_config=${main_config/__DOWNLINK__/$downlink}
        
        local congestion_choice=$(read_input_with_options "是否启用拥塞控制 (congestion)?" "1" "是 (true)" "否 (false, 推荐)")
        if [[ "$congestion_choice" == "1" ]]; then 
            main_config=${main_config/__CONGESTION__/true}
        else 
            main_config=${main_config/__CONGESTION__/false}
        fi
        
        local read_buf=$(read_input "请输入 readBufferSize (读取缓冲区 MB)" "2"); replacements["__READ_BUF__"]="$read_buf"
        main_config=${main_config/__READ_BUF__/$read_buf}
        
        local write_buf=$(read_input "请输入 writeBufferSize (写入缓冲区 MB)" "2"); replacements["__WRITE_BUF__"]="$write_buf"
        main_config=${main_config/__WRITE_BUF__/$write_buf}
        
        local header_opts=("none" "srtp" "utp" "wechat-video" "dtls" "wireguard")
        local header_idx=$(read_input_with_options "请选择伪装头类型 (header type):" "1" "${header_opts[@]}")
        local header_type="${header_opts[$((header_idx-1))]}"
        main_config=${main_config/__HEADER_TYPE__/$header_type}
