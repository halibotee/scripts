...
HY2_CLIENT_INSECURE="0" # Hysteria2 客户端是否跳过证书验证 (0为否)
...
read -r -d '' XRAY_VLESS_MKCP_TEMPLATE <<'EOM'
{
    ...
    "inbounds": [{
        ...
        "streamSettings": {
            "network": "kcp",
            "kcpSettings": {
                "mtu": __MKCP_MTU__, "tti": __MKCP_TTI__, "uplinkCapacity": __MKCP_UPLINK__, "downlinkCapacity": __MKCP_DOWNLINK__, "congestion": __MKCP_CONGESTION__, "readBufferSize": __MKCP_READ_BUF__, "writeBufferSize": __MKCP_WRITE_BUF__,
                "header": { "type": "__MKCP_HEADER__" },
                "seed": "__MKCP_SEED__"
            }
        },
...
param_select_menu() {
    ...
    echo >&2
    cyan "【配置】$param_name" >&2
    for i in "${!options[@]}"; do
        local option="${options[$i]}"
        if [[ "$option" == "$default_value" ]]; then
            green "  $((i+1))) $option (推荐)" >&2
        else
            echo "  $((i+1))) $option" >&2
        fi
    done
    read -p "请选择 [1-${#options[@]}] (默认: 推荐项): " choice >&2
    ...
    if [[ -z "$choice" ]]; then
        ...
        yellow "无效选择，使用默认值: $default_value" >&2
        echo "$default_value"
    fi
}
...
param_number_input() {
    ...
    read -p "请输入数值 (默认: $default_value): " value >&2
    ...
    if [[ -z "$value" ]]; then
        ...
        yellow "无效输入，使用默认值: $default_value" >&2
        echo "$default_value"
    fi
}
...
param_boolean_input() {
    ...
    read -p "是否启用 (默认: $default_display)? [y/N]: " choice >&2
...
install_acme_sh_client() {
    ...
    local Aemail="$auto_email@gmail.com"
    yellow "已自动生成邮箱: $Aemail"
...
download_kcp_udp_binaries(){
    ...
    if [[ -z "$KCPTUN_LATEST" || -z "$UDP2RAW_LATEST" ]]; then
        red "获取 KCPTUN/UDP2RAW 版本号失败。"
        return 1
    fi
    
    # 确保目录存在
    mkdir -p "$KCPTUN_INSTALL_DIR" || { red "无法创建 KCPTUN 目录"; return 1; }
    mkdir -p "$UDP2RAW_INSTALL_DIR" || { red "无法创建 UDP2RAW 目录"; return 1; }
...
ensure_warp_running() {
    ...
    if systemctl is-active --quiet warp-go 2>/dev/null; then
        green "WARP 服务正在运行 (warp-go)。"
        return 0
    fi
    
    # 尝试通过端口检测
    if lsof -ti:40000 >/dev/null 2>&1; then
        green "检测到 WARP Socks5 端口 40000 正在监听。"
        return 0
    fi
    
    yellow "警告: WARP 服务未运行！"
    read -p "是否现在安装/启动 WARP? [Y/n]: " install_warp
    install_warp=${install_warp:-y}
    
    if [[ "$install_warp" == "y" || "$install_warp" == "Y" ]]; then
        log "正在安装/启动 WARP..."
        if install_warp_yg; then
            green "WARP 已安装并启动。"
            return 0
        else
            red "WARP 安装失败！"
            yellow "提示: 您可以稍后通过菜单 14) 安装WARP，或手动配置。"
            read -p "是否继续创建节点(WARP将不可用)? [y/N]: " continue_anyway
        ...
        yellow "提示: WARP 未启用，节点将无法使用 WARP 分流功能。"
        return 0
    fi
}
...
display_instance_status_line() {
    ...
    case "$type" in
        ...
        "vless_chain")
            ...
            line="$($color "${prefix}VLESS_KCP [${xray_status}] + UDP2RAW [${udp_status}] ${udp_info} (${warp_status})")"
            ;;
        "ss_3_chain_chain")
            full_id="s3c${id}"
...
create_new_instance() {
    ...
    case "$type" in
        hysteria2) 
            ...
            echo
            cyan "--- Hysteria2 协议参数 ---"
            local ignore_bw=$(param_boolean_input "忽略客户端带宽 (ignoreClientBandwidth)" "true" "推荐启用，使用BBR拥塞控制")
            replacements["__IGNORE_CLIENT_BANDWIDTH__"]="$ignore_bw"
        ...
        udp2raw) TITLE="UDP2RAW"; INSTALL_DIR="$UDP2RAW_INSTALL_DIR"; SERVICE_PREFIX="udp2raw"; SYSTEMD_SERVICE_NAME="ax-udp2raw"; FILE_EXT="conf"; CONFIG_TEMPLATE="$UDP2RAW_CONFIG_TEMPLATE" 
            ...
            # 交互式参数配置
            local raw_mode=$(param_select_menu "原始模式 (raw-mode)" "faketcp" "faketcp" "udp" "icmp")
            replacements["__RAW_MODE__"]="$raw_mode"
            
            local cipher_mode=$(param_select_menu "加密模式 (cipher-mode)" "aes128cbc" "aes128cbc" "xor" "none")
            replacements["__CIPHER_MODE__"]="$cipher_mode"
            
            local auth_mode=$(param_select_menu "认证模式 (auth-mode)" "hmac_sha1" "hmac_sha1" "md5" "simple")
        ...
        kcptun) TITLE="KCPTUN"; INSTALL_DIR="$KCPTUN_INSTALL_DIR"; SERVICE_PREFIX="kcptun"; SYSTEMD_SERVICE_NAME="ax-kcptun"; FILE_EXT="json"; CONFIG_TEMPLATE="$KCPTUN_CONFIG_JSON_TEMPLATE" 
            ...
            cyan "--- KCPTUN 参数配置 ---"
            # 交互式参数配置
            local mode=$(param_select_menu "加速模式 (mode)" "fast3" "fast3" "fast2" "fast" "normal")
            replacements["__MODE__"]="$mode"
            
            local crypt=$(param_select_menu "加密方式 (crypt)" "aes-128" "aes-128" "aes-192" "aes" "salsa20" "none")
            replacements["__CRYPT__"]="$crypt"
            
            local mtu=$(param_number_input "MTU" "1350" "建议: 1350-1400")
            replacements["__MTU__"]="$mtu"
            
            local sndwnd=$(param_number_input "发送窗口 (sndwnd)" "256" "服务端发送窗口大小")
            replacements["__SNDWND__"]="$sndwnd"
            
            local rcvwnd=$(param_number_input "接收窗口 (rcvwnd)" "2048" "客户端接收窗口大小")
            replacements["__RCVWND__"]="$rcvwnd"
            
            local datashard=$(param_number_input "数据分片 (datashard)" "10" "前向纠错数据分片")
            replacements["__DATASHARD__"]="$datashard"
            
            local parityshard=$(param_number_input "校验分片 (parityshard)" "3" "前向纠错校验分片")
...
create_new_xray_instance() {
    ...
    if [[ "$type" == "xray_reality" ]]; then
        ...
        echo
        cyan "--- mKCP 参数配置 ---"
        local mkcp_mtu=$(param_number_input "MTU" "1200" "建议: 1200-1400")
        replacements["__MKCP_MTU__"]="$mkcp_mtu"
        
        local mkcp_tti=$(param_number_input "TTI (传输时间间隔)" "30" "建议: 20-50ms")
        replacements["__MKCP_TTI__"]="$mkcp_tti"
        
        local mkcp_uplink=$(param_number_input "上行容量 (uplinkCapacity)" "25" "单位: MB/s")
        replacements["__MKCP_UPLINK__"]="$mkcp_uplink"
        
        local mkcp_downlink=$(param_number_input "下行容量 (downlinkCapacity)" "100" "单位: MB/s")
        replacements["__MKCP_DOWNLINK__"]="$mkcp_downlink"
        
        local mkcp_congestion=$(param_boolean_input "拥塞控制 (congestion)" "true" "推荐启用")
        replacements["__MKCP_CONGESTION__"]="$mkcp_congestion"
        
        local mkcp_read_buf=$(param_number_input "读取缓冲 (readBufferSize)" "4" "单位: MB")
        replacements["__MKCP_READ_BUF__"]="$mkcp_read_buf"
        
        local mkcp_write_buf=$(param_number_input "写入缓冲 (writeBufferSize)" "4" "单位: MB")
        replacements["__MKCP_WRITE_BUF__"]="$mkcp_write_buf"
        
        local mkcp_header=$(param_select_menu "数据包伪装类型 (header)" "wechat-video" "wechat-video" "dtls" "wireguard" "srtp" "utp" "none")
        replacements["__MKCP_HEADER__"]="$mkcp_header"
        
        # 生成UUID和Seed
        echo
        cyan "--- 生成 UUID 和 mKCP Seed ---"
        local uuid=$(generate_strong_password); replacements["__UUID__"]="$uuid"
        ...
        # Shadowsocks 参数配置
        echo
        cyan "--- Shadowsocks 参数配置 ---"
        local ss_method=$(param_select_menu "加密方式 (method)" "2022-blake3-aes-256-gcm" "2022-blake3-aes-256-gcm" "2022-blake3-aes-128-gcm" "2022-blake3-chacha20-poly1305")
...
generate_xray_ss_link() {
    ...
    local link="ss://${user_info}@${ip}:${port}#Xray_SS_${ip}_${id}"
    echo "$link"
}
...
# [NEW]# 获取 3 组件串联实例ID列表
get_3_chain_instances() {
    local instances1=$(ls -1 "$XRAY_INSTALL_DIR"/xray_chain_ss_kcp_udp2raw_*.json 2>/dev/null | sed -E 's/.*chain_ss_kcp_udp2raw_([0-9]+).*/\1/')
    local instances2=$(ls -1 "$KCPTUN_INSTALL_DIR"/kcptun_chain_ss_kcp_udp2raw_*.json 2>/dev/null | sed -E 's/.*chain_ss_kcp_udp2raw_([0-9]+).*/\1/')
    local instances3=$(ls -1 "$UDP2RAW_INSTALL_DIR"/udp2raw_chain_ss_kcp_udp2raw_*.conf 2>/dev/null | sed -E 's/.*chain_ss_kcp_udp2raw_([0-9]+).*/\1/')
...
view_chain_client_config_3() {
    local id_num=$1
    local id="chain_ss_kcp_udp2raw_${id_num}"
    
    local ss_conf="$XRAY_INSTALL_DIR/xray_${id}.json"
    local kcp_conf="$KCPTUN_INSTALL_DIR/kcptun_${id}.json"
    local udp_conf="$UDP2RAW_INSTALL_DIR/udp2raw_${id}.conf"
    
    if [[ ! -f "$ss_conf" || ! -f "$kcp_conf" || ! -f "$udp_conf" ]]; then
...
start_new_chain_instance_3() {
    local i=1
    while true; do if [[ ! -f "$XRAY_INSTALL_DIR/xray_chain_ss_kcp_udp2raw_${i}.json" ]]; then break; fi; i=$((i + 1)); done
    
    local chain_id="chain_ss_kcp_udp2raw_${i}"
    ...
    kcp_config=${kcp_config/__MODE__/fast3}
    ...
    udp_config=${udp_config//__RAW_MODE__/faketcp}
    udp_config=${udp_config//__CIPHER_MODE__/aes128cbc}
    udp_config=${udp_config//__AUTH_MODE__/hmac_sha1}
...
get_chain_instances() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        instances1=$(ls -1 "$HY2_INSTALL_DIR"/hy2_chain_hy2_udp2raw_*.yaml 2>/dev/null | sed -E 's/.*chain_hy2_udp2raw_([0-9]+).*/\1/')
        instances2=$(ls -1 "$UDP2RAW_INSTALL_DIR"/udp2raw_chain_hy2_udp2raw_*.conf 2>/dev/null | sed -E 's/.*chain_hy2_udp2raw_([0-9]+).*/\1/')
    else # vless
        instances1=$(ls -1 "$XRAY_INSTALL_DIR"/xray_chain_vless_kcp_udp2raw_*.json 2>/dev/null | sed -E 's/.*chain_vless_kcp_udp2raw_([0-9]+).*/\1/')
        instances2=$(ls -1 "$UDP2RAW_INSTALL_DIR"/udp2raw_chain_vless_kcp_udp2raw_*.conf 2>/dev/null | sed -E 's/.*chain_vless_kcp_udp2raw_([0-9]+).*/\1/')
    fi
    echo "$instances1 $instances2" | tr ' ' '\n' | sort -un
}

# 获取 3 组件串联实例ID列表 (SS+KCP+UDP2RAW)
...
view_chain_client_config() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        id_prefix="chain_hy2_udp2raw_"; title="Hysteria2"; main_conf_path="$HY2_INSTALL_DIR/hy2_${id_prefix}${id_num}.yaml"; client_listen_addr="$CLIENT_UDP2RAW_LISTEN_ADDR"
    else
        id_prefix="chain_vless_kcp_udp2raw_"; title="VLESS_KCP"; main_conf_path="$XRAY_INSTALL_DIR/xray_${id_prefix}${id_num}.json"; client_listen_addr="$CLIENT_VLESS_UDP2RAW_LISTEN_ADDR"
    fi
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        display_title="[VLESS_KCP+UDP2RAW]"
        ...
        # [修改点] 更改了节点备注格式：Chain_VLESSmKCP_... +UDP2RAW
        sub_link="vless://${uuid}@${client_udp2raw_host}:${client_udp2raw_port}?type=kcp&security=none&headerType=${header_type}&seed=${seed}#Chain_VLESSmKCP_${host_label}_${id}+UDP2RAW"
    fi
...
start_new_chain_instance() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        id_prefix="chain_hy2_udp2raw_"; title="Hysteria2+UDP"; main_conf_dir="$HY2_INSTALL_DIR"; main_conf_template="$HYSTERIA2_CONFIG_YAML_TEMPLATE"
        while true; do if [[ ! -f "$main_conf_dir/hy2_${id_prefix}${i}.yaml" ]]; then break; fi; i=$((i + 1)); done
    else # vless
        id_prefix="chain_vless_kcp_udp2raw_"; title="VLESS_KCP+UDP"; main_conf_dir="$XRAY_INSTALL_DIR"; main_conf_template="$XRAY_VLESS_MKCP_TEMPLATE"
        while true; do if [[ ! -f "$main_conf_dir/xray_${id_prefix}${i}.json" ]]; then break; fi; i=$((i + 1)); done
    fi
    ...
    cyan "========================================"
    if [[ "$chain_type" == "hy2" ]]; then
        cyan "  配置 HYSTERIA2+UDP2RAW 串联实例"
    else
        cyan "  配置 VLESS+mKCP+UDP2RAW 串联实例"
    fi
    cyan "========================================"
    echo
    ...
    # 监听端口配置
    cyan "--- 外部监听端口 (UDP2RAW) ---"
    read -p "请输入对外监听端口 (留空则随机): " udp2raw_listen_port
    if [[ -z "$udp2raw_listen_port" ]]; then 
        udp2raw_listen_port=$(find_available_port); 
        green "已自动选择对外端口: $udp2raw_listen_port"
    fi
    ...
    echo
    cyan "--- UDP2RAW 协议参数 ---"
    local udp2raw_raw_mode=$(param_select_menu "原始模式 (raw-mode)" "faketcp" "faketcp" "udp" "icmp")
    local udp2raw_cipher=$(param_select_menu "加密模式 (cipher-mode)" "aes128cbc" "aes128cbc" "xor" "none")
    local udp2raw_auth=$(param_select_menu "认证模式 (auth-mode)" "hmac_sha1" "hmac_sha1" "md5" "simple")
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        echo
        cyan "--- HYSTERIA2 协议参数 ---"
        local hy2_ignore_bw=$(param_boolean_input "忽略客户端带宽 (ignoreClientBandwidth)" "true" "推荐启用")
        ...
        main_config="${main_config//__IGNORE_CLIENT_BANDWIDTH__/${hy2_ignore_bw}}"
        ...
        # 设置参数值
        main_config=${main_config/__IGNORE_CLIENT_BANDWIDTH__/true}
        main_config=${main_config/__MKCP_MTU__/$mkcp_mtu}
        main_config=${main_config/__MKCP_TTI__/$mkcp_tti}
        main_config=${main_config/__MKCP_UPLINK__/$mkcp_uplink}
        main_config=${main_config/__MKCP_DOWNLINK__/$mkcp_downlink}
        main_config=${main_config/__MKCP_CONGESTION__/$mkcp_congestion}
        main_config=${main_config/__MKCP_READ_BUF__/$mkcp_read_buf}
        main_config=${main_config/__MKCP_WRITE_BUF__/$mkcp_write_buf}
        main_config=${main_config/__MKCP_HEADER__/$mkcp_header}
    ...
    temp_udp_config="${temp_udp_config/__RAW_MODE__/$udp2raw_raw_mode}"
    temp_udp_config="${temp_udp_config/__CIPHER_MODE__/$udp2raw_cipher}"
    temp_udp_config="${temp_udp_config/__AUTH_MODE__/$udp2raw_auth}"
...
manage_chain_instance() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        id_prefix="vc"; title="VLESS_KCP"; service1_name="ax-xray"; main_conf_path="$XRAY_INSTALL_DIR/xray_vc${id_num}.json"
    fi
    ...
    while true; do
        ...
        if [[ "$chain_type" == "hy2" ]]; then
            ...
            $color "状态: VLESS_KCP [${s1_status}] + UDP2RAW [${s2_status}] $(dim "$udp2raw_info")"
        fi
...
# 重启所有服务
restart_all_services(){ log "正在重启所有正在运行的实例..."; systemctl restart ax-kcptun@*.service ax-udp2raw@*.service ax-hysteria2@*.service ax-xray@*.service 2>/dev/null; green "操作完成！"; sleep 2; }
...
install_warp_yg() {
    echo "----------------------------------------------------------------"
    log "正在下载 WARP-Socks5 核心文件..."
    
    # 只下载脚本，不执行
    if wget --no-check-certificate -O warp-socket5.sh "https://raw.githubusercontent.com/halibotee/scripts/main/warp-socket5.sh"; then
        chmod +x warp-socket5.sh
        green "WARP-Socks5 脚本已下载到当前目录。"
        echo "----------------------------------------------------------------"
        cyan "提示: 如需安装 WARP，请手动执行: ./warp-socket5.sh"
        echo "----------------------------------------------------------------"
        return 0
    else
        echo "----------------------------------------------------------------"
        red "错误：下载 warp-socket5.sh 失败。"
        log "即将返回主菜单..."
        return 1
    fi
}
...
acme_certificate_menu() {
    while true; do
        trap 'echo -e "\n\n${yellow}操作已取消, 返回主菜单...${reset}"; sleep 1; return' SIGINT
        clear
        echo "=================================="
        echo "      ACME 证书管理"
        echo "=================================="
...
initial_check_and_install() {
    ...
    if [[ "$has_configs" == true && ( "$kcp_udp_ok" == false || "$hy2_ok" == false || "$xray_ok" == false ) ]]; then
        ...
        # 自动安装 ACME.sh
        echo
        log "正在自动安装 ACME.sh 证书管理工具..."
        if install_acme_sh_client; then
            green "ACME.sh 安装成功！"
        else
            yellow "ACME.sh 安装失败，可稍后通过菜单安装。"
        fi
        
        # 自动安装 WARP
        echo
        log "正在自动安装 WARP-Socks5 分流工具..."
        if install_warp_yg; then
            green "WARP 安装成功！"
        else
            yellow "WARP 安装失败，可稍后通过菜单安装。"
        fi
    ...
    while true; do
        ...
        if [[ -n "${QUICK_MANAGE_MAP_ID[$choice]}" ]]; then
            ...
            case "$type" in
                ...
                "kcptun") manage_instance_menu "kcptun" "$real_id" "ax-kcptun@${real_id}" "$KCPTUN_INSTALL_DIR/kcptun_${real_id}.json" ;;
            esac
            continue
        fi
