...
congestion:
  type: __CONGESTION_TYPE__
  down: __CONGESTION_DOWN__
  up: __CONGESTION_UP__
sniff:
  enable: true
  timeout: 2s
  rewriteDomain: true
...
start_new_chain_instance() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        if [[ "$congestion_enabled" == "true" ]]; then
            ...
            local congestion_block="type: $congestion_type\n  down: $congestion_down\n  up: $congestion_up"
            main_config=$(echo -e "$main_config" | sed "s/congestion:/congestion:\n  $congestion_block/")
        else
            main_config=$(echo -e "$main_config" | sed "s/congestion:/congestion: null/")
        fi
    ...
    while true; do
        ...
        if [[ -n "${QUICK_MANAGE_MAP_ID[$choice]}" ]]; then
            ...
            case "$type" in
                ...
                "kcptun") manage_instance_menu "kcptun" "$real_id" "kcptun@${real_id}" "$KCP_INSTALL_DIR/kcptun_${real_id}.json" ;;
            esac
            continue
        fi
