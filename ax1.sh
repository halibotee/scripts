...
collect_kcptun_params() {
    ...
    # 用户输入 true (disable compression) -> nocomp=true
    # 用户输入 false (enable compression) -> nocomp=false
...
generate_hy2_subscription_link() {
    ...
    local ignore_bw=$(grep -Po '(?<=ignoreClientBandwidth: ).*' "$conf" | tr -d '[:space:]')
    
    # [New] 提取 ignoreClientBandwidth 并添加到备注
    local remark="hy2_${host_label}_${id}"
    if [[ "$ignore_bw" == "true" ]]; then
        remark="${remark}(ignore_bw:true)"
    fi

    echo "hysteria2://${password}@${ip}:${port}?sni=${sni}&insecure=${insecure_flag}#${remark}"
}
...
generate_xray_mkcp_link() {
    ...
    # [New] 添加 congestion 参数到链接 (虽然标准链接可能不支持直接显示，但可以加在备注里)
    local remark="vless_mkcp_${ip}_${id}"
    if [[ "$congestion" == "true" ]]; then
        remark="${remark}(congestion:on)"
    fi
    
    echo "vless://${uuid}@${ip}:${port}?type=kcp&security=none&headerType=${header_type}&seed=${seed}#${remark}"
}
...
view_chain_client_config_3() {
    ...
    local sub_link="ss://${ss_user_info}@${ss_host}:${ss_port}#Chain_ss_kcp_${ip}_${id}+UDP2RAW(kcp:${kcp_mode},${kcp_crypt},nocomp:${kcp_nocomp}|udp:${udp_raw_mode},${udp_cipher_mode},${udp_auth_mode})"
    green "$sub_link"; echo
...
view_chain_client_config() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        sub_link="hysteria2://${hy2_password}@${client_udp2raw_host}:${client_udp2raw_port}?sni=${sni}&insecure=${insecure_flag}#Chain_hy2_${host_label}_${id}+UDP2RAW(raw:${udp_raw_mode},cipher:${udp_cipher_mode},auth:${udp_auth_mode})"
    else # vless
        ...
        sub_link="vless://${uuid}@${client_udp2raw_host}:${client_udp2raw_port}?type=kcp&security=none&headerType=${header_type}&seed=${seed}#Chain_vless_kcp_${host_label}_${id}+UDP2RAW(raw:${udp_raw_mode},cipher:${udp_cipher_mode},auth:${udp_auth_mode})"
    fi
