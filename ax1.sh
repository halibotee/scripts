...
collect_udp2raw_params() {
    ...
    read -p "cipher_mode [aes128cbc/xor/none] (默认 aes128cbc): " cipher_mode
    cipher_mode=${cipher_mode:-aes128cbc}
    
    read -p "auth_mode [hmac_sha1/simple/md5] (默认 hmac_sha1): " auth_mode
...
collect_kcptun_params() {
    cyan "--- KCPTUN 配置 ---"
    read -p "mode [fast/fast2/fast3/normal/manual] (默认 $KCPTUN_DEFAULT_MODE): " mode
    mode=${mode:-$KCPTUN_DEFAULT_MODE}
    
    read -p "crypt [aes-128/aes-192/aes-256] (默认 aes-128): " crypt
    crypt=${crypt:-aes-128}
    
    read -p "tcp [true/false] (默认 false): " tcp_enabled
    tcp_enabled=${tcp_enabled:-false}
    if [[ "$tcp_enabled" != "true" ]]; then
        tcp_enabled="false"
    fi
    
    read -p "nocomp [true/false] (默认 false): " compression_enabled
