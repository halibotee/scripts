...
# Modified with ACME integration

# --- 1. 核心全局变量与脚本版本 ---
SCRIPT_VERSION="1.2.0" # 脚本版本号
...
release_80_port() {
    ...
    if [ -n "$pid" ]; then
        ...
        read -p "是否尝试强行释放 80 端口? [y/N]: " kill_confirm
...
create_new_instance() {
    ...
    case "$type" in
        hysteria2) 
            ...
            # --- [新] ACME 与伪装逻辑 ---
            read -p "是否使用 ACME (Let's Encrypt) 证书? (否则将使用自签名证书) [y/N]: " use_acme
