...
HY2_CLIENT_INSECURE="0" # Hysteria2 客户端是否跳过证书验证 (1为是)
...
param_select_menu() {
    ...
    read -p "请选择 [1-${#options[@]}] (默认: 推荐项): " choice
    ...
    if [[ -z "$choice" ]]; then
        ...
        yellow "无效选择，使用默认值: $default_value"
        echo "$default_value"
    fi
}
...
param_number_input() {
    ...
    read -p "请输入数值 (默认: $default_value): " value
    ...
    if [[ -z "$value" ]]; then
        ...
        yellow "无效输入，使用默认值: $default_value"
        echo "$default_value"
    fi
}
...
param_boolean_input() {
    ...
    read -p "是否启用? (默认: $default_display): " choice
    choice=$(echo "$choice" | tr '[:upper:]' '[:lower:]')
    
    if [[ -z "$choice" ]]; then
        echo "$default_value"
    elif [[ "$choice" == "y" || "$choice" == "yes" || "$choice" == "是" || "$choice" == "true" ]]; then
        echo "true"
    else
        echo "false"
    fi
