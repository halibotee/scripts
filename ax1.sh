...
# Modified with ACME integration (Standalone + Chain)

# --- 1. 核心全局变量与脚本版本 ---
SCRIPT_VERSION="1.2.1" # 脚本版本号
...
# [新] ACME 相关配置
ACME_SH_INSTALL_DIR="/root/.acme.sh/acme.sh" # acme.sh 客户端的默认路径
AX_CERT_DIR="/etc/ax-certs"                 # ax 脚本专用的共享证书存储目录
...
ax_get_certificate() {
    ...
    if [ "$acme_mode" == "1" ]; then
        ...
        case $dns_provider in
            1) # Cloudflare
                read -p "请输入 Cloudflare Global API Key: " GAK; echo
                export CF_Key="$GAK"
                ...
                ;;
            2) # DNSPod
                read -p "请输入 DNSPod DP_Id: " DPID; echo
                export DP_Id="$DPID"
                read -p "请输入 DNSPod DP_Key: " DPKEY; echo
                export DP_Key="$DPKEY"
                dns_api_cmd="--dns dns_dp"
                ;;
            3) # Aliyun
                read -p "请输入 Aliyun Ali_Key: " ALKEY; echo
                export Ali_Key="$ALKEY"
                read -p "请输入 Aliyun Ali_Secret: " ALSER; echo
        ...
        hysteria2) 
            ...
            if [[ "$use_acme" == "y" || "$use_acme" == "Y" ]]; then
                ...
                if [ -f "$AX_CERT_DIR/$domain_name/fullchain.cer" ]; then
                    green "ACME 成功. Hysteria2 将使用 $domain_name"
                    cert_path="$AX_CERT_DIR/$domain_name/fullchain.cer"
                    key_path="$AX_CERT_DIR/$domain_name/private.key"
                    sni="$domain_name"
                    masquerade_url="https://$domain_name" # 伪装目标使用自己的域名
                else
                    yellow "ACME 申请/查找失败, 回退到自签名证书..."
                    use_acme="n" # 强制回退
                fi
            fi
            ...
            read -p "是否启用 WARP SOCKS5 分流 (默认"不启用")？[y/N]: " enable_warp
...
generate_hy2_subscription_link() {
    ...
    echo "hysteria2://${password}@${ip}:${port}?sni=${sni}&insecure=${insecure_flag}#hy2_${ip}_${id}"
}
...
view_chain_client_config() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        sub_link="hysteria2://${hy2_password}@${client_udp2raw_host}:${client_udp2raw_port}?sni=${sni}&insecure=${insecure_flag}#hy2_chain_${ip}_${id}"
    else # vless
...
uninstall_all() {
    ...
    if [[ "$nuke_choice" == "y" || "$nuke_choice" == "Y" ]]; then
        log "步骤 3: 执行彻底清理..."
        rm -rf "$KCP_UDP_INSTALL_DIR" "$HY2_INSTALL_DIR" "$XRAY_INSTALL_DIR" "$AX_CERT_DIR"
        green "程序和配置文件目录已删除。"
        log "步骤 4: 删除生成的证书..."
...
main_menu(){
    ...
    while true; do
        ...
        echo " 12) 检查更新程序" 
        cyan "--- 工具管理 ---"
        echo " 13) 安装 VPS一键优化脚本"
        echo " 14) 安装 warp-Socks5一键安装脚本"
        echo "----------------------------------"   
        echo " 99) 卸载"
        echo " 0) 退出"
