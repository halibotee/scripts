...
install_sys_opt() {
    echo "----------------------------------------------------------------"
    log "运行 VPS 一键优化脚本 (vps_optimizert.sh) ..."
    ...
    if [ -f "$AX_TOOLS_DIR/vps_optimizert.sh" ]; then
         "$AX_TOOLS_DIR/vps_optimizert.sh"
         echo "----------------------------------------------------------------"
         green "VPS 一键优化脚本执行完毕。"
    else
         red "错误：脚本文件未找到。"
    fi
    log "即将返回主菜单..."
...
install_warp_yg() {
    ...
    if [ -f "$AX_TOOLS_DIR/warp-socket5.sh" ]; then
         "$AX_TOOLS_DIR/warp-socket5.sh"
         echo "----------------------------------------------------------------"
         green "warp-socket5.sh 脚本执行完毕。"
    else
         red "错误：脚本文件未找到。"
    fi
    log "即将返回主菜单..."
...
uninstall_all() {
    ...
    if systemctl list-units --full -all | grep -q "warp-svc.service"; then
        echo
        yellow "检测到 Cloudflare WARP 服务 (warp-svc) 正在运行。"
        read -p "是否连同 WARP 一起卸载？(删除 cloudflare-warp 包) [y/N]: " remove_warp
    fi
    ...
    if [[ "$remove_acme" == "y" || "$remove_acme" == "Y" ]]; then
        ...
        if [[ -f "/root/.acme.sh/acme.sh" ]]; then
            "/root/.acme.sh/acme.sh" --uninstall --nocolor >/dev/null 2>&1
        fi
        rm -rf "/root/.acme.sh"
        green "acme.sh 已卸载。"
    fi
    ...
    echo
    green "==== 卸载操作已完成 ===="
    yellow "提示：为了系统稳定性，基础依赖包 (curl, openssl, jq 等) 未被移除。"
    if [[ "$deep_clean" != "y" && "$deep_clean" != "Y" ]]; then
