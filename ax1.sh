...
create_new_xray_instance() {
    ...
    if [[ "$type" == "xray_reality" ]]; then
        ...
        if [[ -z "$key_pair" ]]; then
            red "错误: 无法运行 Xray 生成密钥。请检查 Xray 是否正确安装。"
            return 1
        fi
        
        # 使用更稳健的提取方式 (处理 : 后的空格)
        local private_key=$(echo "$key_pair" | grep "Private" | awk -F':' '{print $2}' | tr -d '[:space:]')
        local public_key=$(echo "$key_pair" | grep "Public" | awk -F':' '{print $2}' | tr -d '[:space:]')
