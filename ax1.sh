...
# --- JQ Patch Start ---
    ...
    # --- JQ Patch End ---
        ...
        # 仅对 JSON 文件类型的配置进行转义处理 (防止破坏 YAML 或 CLI 参数)
        if [[ "$FILE_EXT" == "json" ]]; then
            safe_val=$(escape_json_val "$raw_val")
        fi
    ...
    if [[ "$FILE_EXT" == "json" ]]; then
        if command -v jq >/dev/null; then
            if ! echo "$temp_config" | jq . >/dev/null 2>&1; then
                red "严重错误: 生成的配置文件 JSON 语法校验失败！"
                yellow "可能是某个变量包含了无法转义的特殊字符。保留配置文件供调试。"
                # 不退出，但给出强烈警告
            fi
        fi
    fi
...
create_new_xray_instance() {
    ...
    if [[ "$type" == "xray_reality" ]]; then
        ...
        local method=$(read_input "请输入加密方式" "$SS_DEFAULT_METHOD"); replacements["__SS_METHOD__"]="$method"
        local password=$(handle_password_input "shadowsocks"); replacements["__SS_PASSWORD__"]="$password"
    fi
...
start_new_chain_instance_3() {
    ...
    if [[ -z "$udp2raw_listen_port" ]]; then 
        ...
        if check_port_occupied "$udp2raw_listen_port"; then red "端口 $udp2raw_listen_port 已被占用！"; return; fi
    fi
    ...
    local ss_method=$(read_input "请输入 Shadowsocks 加密方式" "$SS_DEFAULT_METHOD")
...
uninstall_all() {
    ...
    systemctl stop "ax-kcptun@*.service" "ax-udp2raw@*.service" "hysteria2@*.service" "ax-xray@*.service" "hysteria-server@*.service" 2>/dev/null
    systemctl disable "ax-kcptun@*.service" "ax-udp2raw@*.service" "hysteria2@*.service" "ax-xray@*.service" "hysteria-server@*.service" 2>/dev/null
    green "服务已停止并禁用。"
...
dispatch_management_menu() {
    ...
    case "$type" in
        ...
        "hysteria2") manage_instance_menu "hysteria2" "$id" "hysteria2@${id}" "$HY2_INSTALL_DIR/hy2_${id}.yaml" ;;
