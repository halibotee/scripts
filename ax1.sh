...
generate_hy2_subscription_link() {
    ...
    # [New] 提取 ignoreClientBandwidth 并添加到 URL 参数
    local extra_params="&ignoreClientBandwidth=$ignore_bw"

    echo "hysteria2://${password}@${ip}:${port}?sni=${sni}&insecure=${insecure_flag}${extra_params}#hy2_${host_label}_${id}"
}
...
view_chain_client_config() {
    ...
    if [[ "$chain_type" == "hy2" ]]; then
        ...
        sub_link="vless://${uuid}@${client_udp2raw_host}:${client_udp2raw_port}?type=kcp&security=none&headerType=${header_type}&seed=${seed}&udp_raw_mode=${udp_raw_mode}&udp_cipher_mode=${udp_cipher_mode}&udp_auth_mode=${udp_auth_mode}#Chain_vless_kcp_${host_label}_${id}+UDP2RAW"
    fi
