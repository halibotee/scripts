		...
		intoQueue({
			...
			success: function(response) {
				...
				if (response.search("BBABBBBC") != -1) {
					...
					count_down_close1();
					var _pkg_arch = "arm32";
					var _pkg_m = response.match(/架构:\s*(\S+)/);
					if (_pkg_m) _pkg_arch = _pkg_m[1];
					var a = "http://"+window.location.hostname+"/_temp/"+"mc_package_"+_pkg_arch+".tar.gz";
					var downloadA = document.createElement('a');
					downloadA.href = a;
					downloadA.download = "mc_package_"+_pkg_arch+".tar.gz";
					downloadA.click();
					window.URL.revokeObjectURL(downloadA.href);
					return true;
				}
				if (_pkg_respLen == response.length) {
					_pkg_noChange++;
				} else {
					_pkg_noChange = 0;
				}
				if (_pkg_noChange > 1000) {
					return false;
				} else {
					setTimeout("poll_pkg_log();", 500);
				}
				retArea.value = response.replace("BBABBBBC", " ");
				retArea.scrollTop = retArea.scrollHeight;
				_pkg_respLen = response.length;
			},
			error: function() {
				setTimeout("poll_pkg_log();", 500);
			}
	...
	function package_plugin() {
		...
		showMCLoadingBar(120);
		var id = parseInt(Math.random() * 100000000);
		var postData = {"id": id, "method": "clash_package.sh", "params":["package"], "fields": {}};
		intoQueue({
			type: "POST",
			url: "/_api/",
			async: true,
									...
									<tr>
										<td bgcolor="#4D595D" colspan="3" valign="top">
											...
											<div class="SimpleNote" id="head_illustrate"><i></i>
												...
												<p id="showmsg4" style="display:none"></p>
												<p id="showmsg5" style="display:none"></p>
												<p id="showmsg6" style="display:none"></p>
												<p id="showmsg7" style="display:none"></p>
												<p id="showmsg8" style="display:none"></p>
												<p id="showmsg9" style="display:none"></p>
												<p id="showmsg10" style="display:none"></p>
											</div>
																			...
																			<table style="margin:10px 0px 0px 0px;" width="100%" border="1" align="center" cellpadding="4" cellspacing="0" bordercolor="#6b8fa3" class="FormTable" id="merlinclash_switch_table">
																				...
																				<tr>
																					...
																					<td colspan="2">
																						...
																						<a type="button" style="vertical-align: middle; cursor:pointer;" id="clashdata-btn-package" class="ks_btn" onclick="package_plugin()" >打包插件</a>
																					</td>
																				</tr>
																				<tr>
