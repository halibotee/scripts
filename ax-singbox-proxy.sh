...
AX_CERT_DIR="/etc/ax-certs"                       # 证书统一存放目录 (已在上面定义)
...
# -----------------------------------------------------------------------------
# Sing-box WARP 分流 outbound 块 (WireGuard 原生)
# 私钥和地址可通过 register_warp_wireguard 自动获取
# -----------------------------------------------------------------------------
read -r -d '' SINGBOX_WARP_WIREGUARD_OUTBOUND <<'EOM'
{
    "type": "wireguard",
    "tag": "warp-out",
    ...
    "local_address": [
        "172.16.0.2/32",
        "2606:4700:110:__WARP_IPV6_SUFFIX__/128"
    ],
    "private_key": "__WARP_PRIVATE_KEY__",
    "peer_public_key": "bmXw+lhU99v9vBykV99EtTVEW9CHs766lSkaPrCOHg=",
    "reserved": [0, 0, 0],
    "mtu": 1280
}
EOM
...
register_warp_wireguard() {
    ...
    local wg_address_v4=$(echo "$api_result" | jq -r '.config.interface.addresses.v4 // "172.16.0.2"' 2>/dev/null)
...
apply_warp_wireguard_config() {
    ...
    local block=$SINGBOX_WARP_WIREGUARD_OUTBOUND
    block=${block/__WARP_PRIVATE_KEY__/$(jq -r '.private_key' "$warp_key_file")}
    block=${block/__WARP_IPV6_SUFFIX__/$(jq -r '.address_v6' "$warp_key_file" | sed 's|2606:4700:110:||;s|/128||')}
    echo "$block"
}
...
# -----------------------------------------------------------------------------
# 生成 Sing-box Shadowsocks 分享链接
# -----------------------------------------------------------------------------
generate_singbox_ss_link() {
    local id=$1; local tag="ss-${id}"
    local ip=$(get_public_ip)
    local conf="$SINGBOX_INSTALL_DIR/singbox.json"
    if [[ ! -s "$conf" ]]; then echo "N/A"; return; fi
    local port=$(jq -r --arg tag "$tag" '.inbounds[] | select(.tag == $tag) | .listen_port' "$conf")
    local password=$(jq -r --arg tag "$tag" '.inbounds[] | select(.tag == $tag) | .password' "$conf")
    local method=$(jq -r --arg tag "$tag" '.inbounds[] | select(.tag == $tag) | .method' "$conf")
    if [[ -z "$port" || "$port" == "null" || -z "$password" || "$password" == "null" ]]; then echo "N/A"; return; fi
    local user_info=$(echo -n "${method}:${password}" | base64 | tr -d '\n' | sed 's/=*$//')
    local link="ss://${user_info}@${ip}:${port}#Singbox_SS_${ip}_${id}"
    echo "$link"
}
