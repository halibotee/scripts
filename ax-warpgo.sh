...
setup_warpgo() {
  ...
  if [ -s "$WARP_CONF" ]; then
    ...
    if grep -q "^PrivateKey = $" "$WARP_CONF"; then
      hint "检测到无效配置，重新注册..."
      rm -f "$WARP_CONF"
    else
      return 0  # 已有有效配置，跳过
  ...
  # 使用 warp-go 自动注册账户并生成配置
  hint "正在注册 WARP 账户..."
  if "$WARP_DIR/warp-go" --register --config="$WARP_CONF" >/dev/null 2>&1; then
    info "✓ WARP 账户注册成功"
    
    # 添加 Socks5 配置
    if ! grep -q "^Socks5" "$WARP_CONF"; then
      hint "正在添加 Socks5 配置..."
      # 在 [Peer] 部分之前插入 Socks5 配置
      sed -i '/^\[Peer\]/i Socks5 = 127.0.0.1:40000\n' "$WARP_CONF"
      info "✓ Socks5 配置已添加"
    fi
  else
    # 如果 --register 失败，尝试手动创建配置让 warp-go 自己注册
    warning "自动注册失败，尝试手动配置..."
    
    cat > "$WARP_CONF" <<EOF
[Account]
Device = warp-go-socks5
PrivateKey = 
Token = 
Type = free
...
EOF
    ...
    # 让 warp-go 更新配置（填充 PrivateKey 和 Token）
    hint "正在更新配置..."
    if "$WARP_DIR/warp-go" --config="$WARP_CONF" --update >/dev/null 2>&1; then
      info "✓ 配置更新成功"
    else
      error "配置失败，请检查网络连接或手动配置"
    fi
  fi
