...
install_warpgo() {
  # 检查是否已安装
  if [ -s "$WARP_CONF" ] && [ -x "$WARP_DIR/warp-go" ]; then
    # 静默跳过（脚本启动时自动检查）
    [ -z "$1" ] && return 0
    # 手动安装时显示信息
    info "warp-go 已安装在: $WARP_DIR"
    info "配置文件: $WARP_CONF"
    hint "如需重新安装，请先使用 -u 卸载"
    return 0
  fi
  ...
  # 下载 warp-go
  hint "正在下载 warp-go..."
  local WARP_GO_VERSION="1.0.8"  # 可以更新为最新版本
...
EOF
  ...
  if "$WARP_DIR/warp-go" --config="$WARP_CONF" --update >/dev/null 2>&1; then
    ...
    warning "WARP 账户注册失败，将使用默认配置"
  fi
  
  # 创建 systemd 服务文件（可选）
  if [ -d /lib/systemd/system ]; then
    hint "正在创建 systemd 服务..."
...
EOF
  ...
  info "安装目录: $WARP_DIR"
  info "配置文件: $WARP_CONF"
  info "Socks5 端口: 127.0.0.1:40000"
  info "========================================="
  echo ""
  
  # 仅在交互模式下询问是否启动（非命令行模式）
  if [ "$1" != "silent" ]; then
    reading "是否立即启动 warp-go Socks5 代理? (Y/n): " START_NOW
...
main() {
  ...
  if [ $# -eq 0 ]; then
    ...
    check_arch
    check_dependencies
    install_warpgo  # 自动检查并安装 warp-go
    check_virt
    ip4_info
    ip6_info
    check_warpgo_status
    ...
    # 有参数，命令行模式：静默安装 warp-go（如未安装）
    install_warpgo "silent"
